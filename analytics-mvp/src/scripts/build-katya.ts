...
function paint(a,src){
  ...
  document.getElementById('top').innerHTML=(a.top_spend||[]).map(function(c,ci){
    ...
    var main='<tr class="ad-exp" data-i="'+ci+'"><td><span class="cf-tg">▸ </span><b>'+c.id+'</b>'+stBadge(c.status)+'</td><td style="font-size:12px">'+iLabel(c.instr)+'</td><td style="color:var(--ink-3);font-size:12px">'+(c.place||'-')+'</td><td class="r">'+fmtRu(ag.sp)+'</td><td class="r">'+fmtRu(ag.om)+'</td><td class="r">'+ag.ord+'</td><td class="r" style="color:'+drrCol(ag.drr)+'">'+ag.drr+'%</td></tr>';
    var sub='';var omM=0,soM=0;
    if(has){
      st.forEach(function(s){
        var art=SKU_MAP[s.sku]||s.sku;var dr=s.om?Math.round(s.sp/s.om*1000)/10:0;
        omM+=(s.omModel||0);soM+=(s.soldModel||0);
        sub+='<tr class="ad-sku" data-p="'+ci+'" style="display:none"><td style="padding-left:26px" title="'+String(s.name||'').replace(/"/g,"&quot;")+'">Основная '+art+'</td><td style="color:var(--ink-3);font-size:11px">'+iLabel(c.instr)+'</td><td style="color:var(--ink-3);font-size:11px">'+(c.place||'-')+'</td><td class="r">'+fmtRu(s.sp)+'</td><td class="r">'+fmtRu(s.om)+'</td><td class="r">'+(s.sold||0)+'</td><td class="r" style="color:'+drrCol(dr)+'">'+dr+'%</td></tr>';
      });
      if(omM>0||soM>0){sub+='<tr class="ad-sku" data-p="'+ci+'" style="display:none"><td style="padding-left:26px;color:var(--ink-3)">Объединённая карточка (др. SKU)</td><td></td><td></td><td class="r">—</td><td class="r">'+fmtRu(omM)+'</td><td class="r">'+soM+'</td><td class="r">—</td></tr>';}
    } else {
      sub='<tr class="ad-sku" data-p="'+ci+'" style="display:none"><td colspan="7" style="padding-left:26px;color:var(--ink-3);font-size:12px">разбивка по SKU недоступна (каталожная кампания или данные ещё грузятся)</td></tr>';
    }
    return main+sub;
  }).join('');
  document.querySelectorAll('#top .ad-exp').forEach(function(tr){tr.onclick=function(){var i=tr.getAttribute('data-i');var open=false;document.querySelectorAll('#top .ad-sku[data-p="'+i+'"]').forEach(function(s){s.style.display=s.style.display==='none'?'':'none';open=s.style.display!=='none';});var tg=tr.querySelector('.cf-tg');if(tg)tg.textContent=open?'▾ ':'▸ ';};});
...
function render(cur,cmp){
  ...
  setAds('load','подгружаю данные рекламы по SKU… (~15 сек)');
  ...
  fetch(ADS_URL,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({dateFrom:cur.from,dateTo:cur.to,withSku:true}),signal:ctrl?ctrl.signal:undefined})
    ...
    .then(a=>{if(my!==lastReq)return;if(a&&a.totals&&(a.totals.spend>0||a.totals.campaigns>0)){paint(a,'live');setAds('ok','готов к работе · данные рекламы обновлены');}else{paint(baked,'baked2');setAds('warn','реклама пустая — показан снимок');}})
    .catch(()=>{clearTimeout(tid);if(my!==lastReq)return;paint(baked,'baked2');setAds('warn','реклама не загрузилась (таймаут/лимит OZON) — показан снимок');});
}`;
