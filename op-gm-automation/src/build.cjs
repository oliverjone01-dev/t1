/* Сборка сайта ОП ГМ на Контур DS 1.2.
...
let pt = Buffer.concat([d0.update(b0.subarray(0, b0.length - 16)), d0.final()]);
...
const APP = ['opgm.js', 'opgm.css', 'legacy.js', 'legacy.css'];
const h = crypto.createHash('sha256');
for(const f of APP){ const s = fs.readFileSync(path.join(__dirname, f)); h.update(s); fs.writeFileSync(path.join(PUB, 'app', f), s); }
for(const f of ['tokens/tokens.css', 'kit/kit.css', 'kit/kit.js', 'kit/charts.js', 'kit/icons.js']) h.update(fs.readFileSync(path.join(ROOT, 'kontur-ds', f)));
