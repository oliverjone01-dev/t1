...
//   6. не брат по объединённой карточке (пока карты нет - по линии, это догадка);
...
// стороны.
//
// ПРАВИЛА 1 И 6 ПРОТИВОРЕЧАТ ДРУГ ДРУГУ, И РАЗВОДЯТСЯ ПО УРОВНЯМ. Правило 1 тянет к похожему
// товару, правило 6 запрещает брата. Именно из-за первого восемь пар из 21 получили контролем
// родню. Решение: правило 1 работает на уровне КАТЕГОРИИ (первый сегмент артикула: GGL
// зеркала, GGT столы, GGM светильники), а правило 6 запрещает совпадение по ЛИНИИ (два
// сегмента) и по карточке. Категория шире линии, поэтому одно другому не мешает.
//
...
export function pickFor(test: string, start: string, inp: PickInput, taken: Set<string>): Pick {
  ...
  const kinKeys = new Set([...allTestArts].map((a) => kinKey(a, cards)));
  ...
  for (const c of inp.candidates) {
    ...
    if (kinKeys.has(kinKey(c, cards))) {
      const own = kinKey(c, cards) === kinKey(test, cards);
      rejected.push({ art: c, rule: own ? 6 : 7, why: own ? "брат тестового товара этой пары" : "родня тестового товара другого теста" });
      continue;
    }
