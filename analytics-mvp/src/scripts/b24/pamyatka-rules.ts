...
const K_DATE = /сегодня|завтра|послезавтра|сейчас|сразу|в течени[ие]\s+(час|дня|суток|\d)|на (этой|текущей|следующей) неделе|до конца (дня|недели)|ниж[еу]|до \d{1,2}|к \d{1,2}|\d{1,2}[:.]\d{2}|\d{1,2}\.\d{2}|понедельник|вторник|сред[уа]|четверг|пятниц|суббот|воскрес|числа|минут/i;
const K_NOTPROM = /с радостью|всегда готов|буду рад|готов[а]? (ответить|помочь)|сообщу,? что|подскажу,? что/i;
// обещание, обусловленное действием КЛИЕНТА («выбирайте - и посчитаю»), - не обещание
// без срока: срок логично называть после ответа (вторая спорная R2 разметки Ивана 04.09)
const K_CLIENTCOND = /(выбирайте|выберите|пришлите|скажите|напишите|сообщите|уточните|отправьте|определитесь|подскажите|скиньте|направьте|дайте знать|как определитесь|как выберете)/i;
  ...
  for (const [k, list] of Object.entries(evByObj)) {
    ...
    for (const e of win) {
      ...
      if (!marker) for (const sent of body.split(/[.!?\n]+/)) {
        if (K_PROMISE.test(sent) && !K_DONE.test(sent) && !K_DATE.test(sent) && !K_NOTPROM.test(sent) && !K_CLIENTCOND.test(sent) && sent.length > 12 && sent.length < 220) {
