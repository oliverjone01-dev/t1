...
'dyn-ym': () => {
  ...
  return head('Визиты по дням', 'Данные Яндекс.Метрики, счётчик '+esc(String(ym.counter.v))+'. Ряд из '+nf(days.length)+' дней.',
   ...
   + ((ym.goals||[]).length ? '' : ym.conv
      ? '<div class="mt-4">'+note('Сколько из этих визитов стали заявками',
          'Из поиска: '+val(ym.conv.organic)+'% визитов оставляют заявку ('+nf(ym.conv.organic.leads)+' на '+nf(ym.conv.organic.visits)+' за 30 дней). Из рекламы: '+val(ym.conv.ad)+'%. Цифры посчитаны по цели счётчика, подробности на экране «Цели и конверсии».','ok')+'</div>'
