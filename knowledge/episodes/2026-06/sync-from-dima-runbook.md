...
**2026-07-02 · 1-й sync**: pulled **13 commits** from `rop-dashboard-v1@c4b505d`. Conflicts: **none** (automatic merge went well). Duration: ~2 минуты. Merge sha `b275d6d`, rebuild v2 sha `0fb8760`.

Диминые правки (все в его территории, никаких пересечений с нашим):
- Карточка «Продажи» в KPI-строке «Отдел продаж»
- Блок «Нераспределённые: разбор» (таблица + сроки + держатели)
- Кликабельные ID (домен портала) + алерт по нераспределённым
- Фильтр по направлению бизнеса (бренду) в шапке
- Серая «Нераспределённые» в рейтинге / хитмапе / бару команды (вне плана)
- Фикс вёрстки матрицы факт×план (компактный формат денег)
- 3 снимка Bitrix24 (30 июня + 2 раза 2 июля)

Файлы тронуты (5): rop-command.template.html (v1), public/rop-command.html (v1 compiled), rop/data/rop.json, build-rop.ts (+2 строки), b24-snapshots.yml (+1 строка).

Файлы **НЕ тронуты** (наше, проверено git diff): все v2, sales-director skill, knowledge/episodes/2026-07/, consent, Kostya-analitycs-2026/.

После merge: v2 пересобран через `npx tsx analytics-mvp/src/scripts/b24/build-rop-v2.ts`. Результат: **22559 сделок (+30), 9576 лидов (+47), 25 opUsers**.
