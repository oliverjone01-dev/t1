...
| CLAUDE.md §7 Anti-Slop | yes (em dash sweep) | 0 violations |
...
| «350+ установок» / «350+ проектов» (lines 274, 277-278 indirect) | без метки | **MINOR GAP** |
| Толщина покрытия 80мкм (line 238) | v8 line 273 - сужает диапазон 60-80 до 80 | MINOR ACCURACY narrowing |
...
- 8 templates все long-form (500-4000 слов). Skill НЕ покрывает short-form. Risk: попытка применить Type 8 (Category SEO 500-1000) на email или KP. Не явно сказано «НЕ для short-form».
...
6. **Iтоговый pickup на Russian-specific SEO** (Yandex Webmaster, Турбо-страницы, Дзен-перелинковка) - source v8 Part 15 был slim на это, но v9 ещё slimer.
...
- Pattern Dantone (Part 10) - non-trivial: маппинг чужой архитектуры на GENGLASS
...
Минусы:
- Risk-23 (overlap с content-factory): признан в шапке но НЕ имеет conflict-resolution rule. Что если obracaja задаче-bounder выпадет «контентный блок для лендинга на 2500 слов»? content-expert или content-factory? GAP.
...
**Severity:** Low-medium
**Location:** Header / Part 2 introductionsmall
...
**Не требуется** (score 8.23 >= 8.0 threshold для GO без debate). Если автор pushback на GAP-1 - дискуссия.
...
```json
{
  ...
  "rework_tz": "1) Add 2-3 Microdata examples or explicit geo-aeo cross-reference in Part 8. 2) Add 'Boundary cases' section with 5-7 examples mapping query → skill (content-expert vs content-factory). 3) Add 'NOT for' line in header. 4) Add [ДАННЫЕ] tag to '350+ installations' on lines 274, 277-278. 5) Restore 60-80мкм range for powder coating thickness on line 238.",
  "verdict": "go",
  "dispute_thread": null,
  "confidence": 0.86
}
...
| Skill | Conflict | Note |
|---|---|---|
| content-factory | ⚠️ overlap | См. GAP-2 |
| brand | OK | Cross-link явный, quick-reference disclaimer |
| encyclopedia / encyclopedia-router | OK | Cross-link в Part 7 |
| geo-aeo | ⚠️ schema overlap | См. GAP-1 + Schema.org templates могут жить в geo-aeo |
