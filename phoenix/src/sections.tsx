...
export function Slovar() {
  const terms: Array<[string, string]> = [
    ...
    ["ВЧ, СЧ, НЧ", "три наши поисковые кампании по типу запроса: ВЧ короткие популярные («перегородки»), СЧ средние («перегородки для зонирования»), НЧ редкие подробные («перегородка гармошка белая два метра»)"],
    ...
    ["Атрибуция", "как мы определяем, какой канал привёл заявку. У нас сверка не закончена с 8 июля, поэтому ставки и бюджеты пока не трогаем"],
...
const S_LABEL: Record<string, [string, string]> = {
  ok: ["сделано", "good"], half: ["частично", "mid"], hold: ["держится", "good"], held: ["не запускалось", "mid"], no: ["не сделано", "bad"],
...
export function PlanIyul() {
  ...
  return (
    <section className="sec">
      ...
      <Reveal>
        <div className="note note--alarm">
          <b className="note__t">Счёт по {D.PLAN_ROWS} строкам таблицы</b>
          <p>{D.PLAN_TALLY.map((x) => `${x.label} ${x.n}`).join(". ")}. Строка E1 закрывает две задачи бэклога,
            поэтому задач проверено {D.PLAN_COVERAGE.checkedTasks}, а строк {D.PLAN_ROWS}.</p>
    ...
    <footer className="foot">
      <Reveal>
        ...
        <div className="verdict">
          ...
          <p style={{ maxWidth: "44ch" }}>Первоисточник этой страницы - текстовый отчёт <a href="../seo/peregorodki-audit.html" style={{ borderBottom: "1px solid var(--brass)" }}>peregorodki-audit.html</a>.
            Он прошёл адверсариальную приёмку ФЕНИКСА за три круга: 6,25 → 7,35 → 8,75, шестнадцать фактических ошибок найдены и исправлены до выпуска.
