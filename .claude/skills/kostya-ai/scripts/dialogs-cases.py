...
PHONE = re.compile(r"(?:\+?\d[\s()-]?){10,14}\d")
...
STRONG = {
    "STRONG_АЛЬТЕРНАТИВА": ("На «дорого» ответили вариантом, а не скидкой", 100,
        ...
        "По отделу альтернатива звучит в 23% таких ответов."),
    ...
    "STRONG_ПРОАКТИВ": ("О сдвиге срока клиент узнал от вас", 90,
        "Так и держать: за апрель-июль ни одна эскалация отдела не началась с сообщения "
        "менеджера. Ваша началась."),
    "STRONG_КВАЛИФИКАЦИЯ": ("Квалифицирующий вопрос задан", 85,
        "Так и держать: вопрос про бюджет встречается в единицах процентов исходящих "
        "по всему отделу, это провал команды, и вы из него выбиваетесь."),
    "STRONG_СРОК": ("Обещание с конкретной датой", 80,
        "Так и держать: из 631 обещания отдела дату содержали 26. Ваше в их числе."),
...
def scan_deal(seq, add):
    ...
    for m in seq:
        ...
        elif PROMISE.search(txt) and CONCRETE.search(txt) and blen >= 40:
...
def main(src, dst, min_out=MIN_OUT):
    ...
    counts = defaultdict(Counter)                    # mgr -> code -> всего срабатываний
    out_n = Counter()
    # Одна и та же переписка попадает в две сделки: клиент ведёт один диалог, а в
    # CRM у него два номера. Без этой проверки в кабинете стоят две одинаковые
    # карточки, и человек справедливо решает, что скрипт считает его дважды.
