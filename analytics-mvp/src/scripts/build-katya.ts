...
// Сборка OZON включена обратно 25.09.2026 (Иван: «в этой сессии мы правим Озон и обновляем данные,
// но это не должно мешать правкам на Маркете»). Разведение по платформе: OZON пишет public/katya*.html
// из data/, Маркет - public/market/ из data-ym/ (PLATFORM=ym). Код сводов Маркета (sv*) страниц
// OZON не касается, поэтому сборки друг другу не мешают.
...
// paidR - платёж ПОКУПАТЕЛЯ по строке (revenue_money): база налога с 23.09.2026 (Катя: «перестрой
// расчет налогов - 15% от суммы оплатил клиент»). Отдельное слагаемое, а не доля поступления:
// поступление уже за вычетом сборов Маркета, и доли между ними у каждой строки своя.
type SaleRow = { d: string; sku: string; rev: number; units: number; ret: number;
  net0?: number; cogsR?: number; shipOur?: number; shipNeedU?: number; shipKnU?: number; paidR?: number };
  ...
  for (const m of months) {
    for (const r of m.rows || []) {
      ...
      salesRows.push({ d: String(r.d || ""), sku: String(r.sku), rev: d > 0 ? (r.price || 0) * n / d : (r.price || 0), units: n, ret: r.units_returned || 0,
        net0, cogsR: r.cogs || 0, shipOur: r.ship_our || 0, paidR: r.revenue_money || 0,
...
// Седьмое слагаемое - платёж покупателя: с 23.09.2026 налог считается от него, и «Товары» обязаны
// брать ту же базу, что свод. Иначе рентабельность артикула снова разошлась бы с ИТОГО свода.
type PnlDay = [number, number, number, number, number, number, number];
...
const PNL_V: Record<string, Record<number, PnlDay>> = {};
{
  const put = (dst: Record<string, Record<number, PnlDay>>, key: string, i: number, v: PnlDay) => {
    const m = (dst[key] ||= {});
    const c = m[i] || (m[i] = [0, 0, 0, 0, 0, 0, 0]);
    for (let k = 0; k < 7; k++) c[k] = (c[k] as number) + v[k];
  };
  for (const r of salesRows) {
    if (r.net0 == null) continue;
    const i = dayIdx(r.d); if (i < 0 || i >= TOTAL) continue;
    const sk = r.sku;
    const v: PnlDay = [r.net0, r.cogsR || 0, r.shipOur || 0, r.shipNeedU || 0, r.shipKnU || 0, r.units || 0, r.paidR || 0];
  ...
  for (const dst of [PNL_D, PNL_V]) for (const k in dst) for (const d in dst[k]!) {
    const c = dst[k]![d]!;
    for (const j of [0, 1, 2, 6]) c[j] = Math.round(c[j] as number);
  }
}
...
function patchDeliveredColumns(html: string): string {
  ...
  must(/function productPeriod\(p, range\)\{/,
    ...
    //   Чистая      = Валовая − АДМ 30% от Поступления − Налоги 15% от платежа покупателя
    ...
    "  if(!days) return null;\n" +
    "  let net = 0, cogs = 0, ship = 0, un = 0, unOur = 0, unKn = 0, paid = 0, hit = 0;\n" +
    "  for(const k in days){\n" +
    "    const i = +k; if(i < range.startIdx || i > range.endIdx) continue;\n" +
    "    const c = days[k]; hit++;\n" +
    "    net  += c[0] - (OHD[i]||0)*c[5];\n" +
    "    cogs += c[1]; ship += c[2]; unOur += c[3]; unKn += c[4]; un += c[5]; paid += c[6]||0;\n" +
    "  }\n" +
    ...
    "  if(!hit) return null;\n" +
    // Налог - от ПЛАТЕЖА ПОКУПАТЕЛЯ, АДМ - от поступления (Катя 23.09.2026). Базы разные нарочно:
    // поступление уже за вычетом сборов Маркета, налог по ним не уменьшается.
    "  const gp = net - cogs - ship, np = gp - net*0.30 - paid*0.15;\n" +
    "  const noVed = unOur - unKn;\n" +
    "  return { net: net, cogs: cogs, ship: ship, gp: gp, np: np, paid: paid,\n" +
...
// --- страница 5: Деньги (ЖИВЫЕ P&L-вебхуки по периоду, fallback - снимок) ---
{
  ...
  // Финансы по SKU (OZON), базис - ДАТА РЕАЛИЗАЦИИ (начисления):
  //   • месяцы, целиком покрытые транзакциями OZON (/v3/finance/transaction/list, мёртв с 08.09.2026), -
  //     из них (pnl_sku_daily): по февраль-август они сходятся с «Начислениями» до рубля;
  //   • дальше (с FIN_CUT) - из отчёта о реализации за день (realization_daily): «Начислено», комиссия и
  //     «К выплате» из документа, а логистика и прочие сборы, которых в отчёте нет, - из accrual/by-day
  //     по SKU по дате начисления (acq_sku_daily: del, oth).
  // Раньше текущий месяц брался из accrual по ДАТЕ ЗАКАЗА, а штуки и СС - по дате доставки: выручка
  // августовских заказов, доставленных в сентябре, не попадала ни в один месяц (шов, аудит 24.09), а
  // после закрытия сентября его выручка ушла бы в мёртвые транзакции. Иван 25.09.2026, п. 2.2, вариант А.
  // FIN_CUT не зависит от сегодняшней даты: это первый день после последнего месяца, целиком покрытого
  // транзакциями, - иначе 1 октября сентябрь снова пошёл бы из мёртвого источника.
  let txMax = "";
  try { for (const l of readFileSync(dp("pnl_sku_daily.ndjson"), "utf-8").trim().split("\n").filter(Boolean)) { const d = String(JSON.parse(l).d); if (d > txMax) txMax = d; } } catch { /* нет транзакций */ }
  const FIN_CUT = (() => {
    if (!txMax) return "0000-00-00";
    const [y, m, d] = txMax.split("-").map(Number) as [number, number, number];
    const last = new Date(Date.UTC(y, m, 0)).getUTCDate();
    const full = d >= last ? { y, m } : (m === 1 ? { y: y - 1, m: 12 } : { y, m: m - 1 }); // последний целый месяц
    return full.m === 12 ? `${full.y + 1}-01-01` : `${full.y}-${String(full.m + 1).padStart(2, "0")}-01`;
  })();
  ...
  try {
    for (const l of readFileSync(dp("pnl_sku_daily.ndjson"), "utf-8").trim().split("\n").filter(Boolean)) {
      ...
      if (IS_OZON && String(r.d) >= FIN_CUT) continue; // после последнего целого месяца - из отчёта о реализации ниже
  ...
  // С FIN_CUT (OZON) - отчёт о реализации за день + сборы по SKU из by-day. Строка той же схемы:
  // начислено = F + G + баллы за скидки (цена продавца) за вычетом возвратов; комиссия = standard_fee;
  // логистика и прочее - by-day; эквайринг и хранение (индексы 4, 5) здесь 0 - таблица берёт их из
  // acq_sku_daily отдельно, как и раньше; к выплате = начислено + все сборы строки.
  ...
  if (IS_OZON) {
    ...
    // в другом месяце. Нет разбивки - на дату последнего сбора по продаже (sd), нет и её - на дату заказа
    // (доли печатаются). Партнёрская доставка rFBS и прочие ITEM-сборы - из by-day.
    ...
    try {
      for (const l of readFileSync(dp("orders_daily.ndjson"), "utf-8").trim().split("\n").filter(Boolean)) {
        ...
        if (Array.isArray(r.dl) && r.dl.length) { for (const [d, x] of r.dl) { if (String(d) < FIN_CUT) continue; logSd += Number(x) || 0; addFee(String(r.sku), String(d), Number(x) || 0, 0); } continue; }
        const d = String(r.sd || r.d); if (d < FIN_CUT) continue;
        if (r.sd) logSd += v; else logOd += v;
        addFee(String(r.sku), d, v, 0);
      }
    } catch { /* нет заказов */ }
    console.log(`katya: логистика OZON с ${FIN_CUT} - по дате начисления ${Math.round(-logSd)} ₽, по дате заказа (дата начисления не пришла) ${Math.round(-logOd)} ₽`);
    const money: Record<string, { acc: number; com: number }> = {};
    try {
      for (const l of readFileSync(dp("realization_daily.ndjson"), "utf-8").trim().split("\n").filter(Boolean)) {
        const r = JSON.parse(l); const sk = String(r.sku), d = String(r.d);
        if (!rdFrom || d < rdFrom) rdFrom = d; if (d > rdTo) rdTo = d;
        (anRday[sk] ||= []).push([d, (Number(r.sold) || 0) - (Number(r.ret) || 0), Number(r.tb) || 0]);
        if (d >= FIN_CUT) money[sk + "|" + d] = { acc: (Number(r.tb) || 0) + (Number(r.bonus) || 0), com: Number(r.com) || 0 };
      }
    } catch { /* нет отчёта за день - с FIN_CUT денег по SKU нет, до первого прогона orders-backfill */ }
    for (const k of new Set([...Object.keys(money), ...Object.keys(fee)])) {
      const [sk, d] = k.split("|") as [string, string];
      const m = money[k] || { acc: 0, com: 0 }, f = fee[k] || { del: 0, oth: 0 };
      const acc = Math.round(m.acc), com = -Math.round(m.com), del = f.del, oth = f.oth;
  ...
  const anTbYmSet = new Set<string>();
  try {
    for (const l of readFileSync(dp("realization_monthly.ndjson"), "utf-8").trim().split("\n").filter(Boolean)) {
      ...
      (anRealSku[sk] ||= []).push([r.ym, r.sold || 0, r.ret || 0, r.tb == null ? null : Number(r.tb)]);
      if (r.tb != null) anTbYmSet.add(r.ym);
    }
  } catch { /* нет файла - реализация по SKU пустая */ }
  // База налога OZON (Иван 25.09.2026): «Реализовано» отчёта о реализации = F «Реализовано на сумму»
  // + G «Выплаты по механикам лояльности партнёров» за вычетом возвратов (J + K). В строке выше -
  // четвёртым элементом (tb). Месяцы, по которым отчёт уже несёт деньги, - AN_TBYM.
  // Для дней без отчёта (текущий месяц, часть месяца) и для таблицы по заказам базы в документе нет:
  // оцениваем её долей «Реализовано» в «Начислено» этого артикула за месяцы с отчётом. Доля около 45%,
  // а не 100%: «Начислено» включает баллы за скидки, которыми OZON доплачивает за покупателя, а в
  ...
  // заказа: оплата покупателя (F по заказу) + G.
  const gfSum: Record<string, [number, number]> = {};
  try { for (const l of readFileSync(dp("realization_monthly.ndjson"), "utf-8").trim().split("\n").filter(Boolean)) { const r = JSON.parse(l); if (r.f == null) continue; const q = (gfSum[String(r.sku)] ||= [0, 0]); q[0] += Number(r.g) || 0; q[1] += Number(r.f) || 0; } } catch { /* нет отчёта */ }
  let gAll2 = 0, fAll2 = 0; const anGf: Record<string, number> = {};
  for (const k in gfSum) { gAll2 += gfSum[k]![0]; fAll2 += gfSum[k]![1]; if (gfSum[k]![1] >= 50_000) anGf[k] = Math.round(gfSum[k]![0] / gfSum[k]![1] * 1e5) / 1e5; }
  const anGfAll = fAll2 > 0 ? Math.round(gAll2 / fAll2 * 1e5) / 1e5 : 0;
  const anTbRate: Record<string, number> = {};
  for (const k in tbSum) { const a = accSum[k] || 0; const rt = a > 0 ? tbSum[k]! / a : NaN; if (a >= 100_000 && rt > 0 && rt < 1.2) anTbRate[k] = Math.round(rt * 1e4) / 1e4; }
  console.log(`katya: база налога - отчёт о реализации за ${anTbYm.join(", ") || "-"}; доля «Реализовано» в «Начислено»: канал ${anTbRateAll}, своя у ${Object.keys(anTbRate).length} SKU`);
  ...
  // «Доставка покупателя» (доход) - тоже из API: в by-day это NON_ITEM без SKU (кабинетный ряд по дням,
  // data/buyer_delivery_daily.ndjson), поэтому per-order/per-SKU ключа нет - разносим по заказам периода
  // пропорционально штукам в render. Итог сходится с отчётом (август by-day 610 435 ≈ отчёт 605 136).
  // Чистый API, весь период, без ручных отчётов.
  const buyerDelivDaily: any[] = []; // [[d, bd>0], ...] - кабинетный итог (эталон периода)
  try {
    for (const l of readFileSync(dp("buyer_delivery_daily.ndjson"), "utf-8").trim().split("\n").filter(Boolean)) {
      const r = JSON.parse(l); if (r.bd) buyerDelivDaily.push([r.d, Math.round(r.bd)]);
    }
  } catch { /* нет файла - доставка покупателя по заказам 0 */ }
  ...
  // логистику и прочие сборы только при доставке, поэтому у летящего заказа их нет и «К выплате»
  // равнялось 100% выручки. Теперь сборы летящего заказа оцениваются долей от выручки по его артикулу:
  // доставленные заказы за последние 120 дней, отдельно по статьям; при выручке артикула меньше
  // 100 000 ₽ - доля канала. Строка помечается «сборы оценкой», итог оценки - под таблицей.
  const feeShare: Record<string, { rev: number; com: number; del: number; oth: number; prt: number }> = {};
  const FEE_K = ["com", "del", "oth", "prt"] as const;
  try {
    const rowsD = readFileSync(dp("orders_daily.ndjson"), "utf-8").trim().split("\n").filter(Boolean).map((l) => JSON.parse(l));
    const maxOrd = rowsD.reduce((m: string, r: any) => (r.d > m ? r.d : m), "");
    const since = new Date(Date.parse(maxOrd + "T00:00:00Z") - 120 * 86400000).toISOString().slice(0, 10);
  ...
  try {
    for (const l of readFileSync(dp("orders_daily.ndjson"), "utf-8").trim().split("\n").filter(Boolean)) {
      ...
      // Базис блока по заказам: ПО ДАТЕ ЗАКАЗА, «заказано минус отмены (и возвраты)». Исключаем только
      // отменённые (cancelled); доставленные и ещё летящие (delivering/awaiting_deliver) входят как
      // «заказано». Отдельная колонка «Доставлено» показывает, сколько из них уже доставлено.
      // Возврата как статуса постинга в FBO нет - возвраты отражаются в начислениях (обратная логистика),
      // отдельным статусом их не вычесть. Это НЕ совпадает с таблицей по артикулам (та - по начислениям).
      const st = String(r.status || "");
      // Отменённый заказ: выручки и штук нет, но OZON успевает списать логистику и прочие сборы -
      // за июль 26 295 ₽ по 64 заказам (аудит «Денег», Иван 25.09.2026). Раньше такие заказы
      // выбрасывались целиком вместе с расходом. Теперь - строкой заказа только с расходами:
      // «Заказано», «Начислено», СС и база налога нулевые, «К выплате» = сумма сборов (< 0).
      if (st === "cancelled") {
      ...
      anOrders.push({
        ...
        nNoFee: (st === "delivered" && !retAmt && Math.round(r.revenue || 0) > 0 && !Math.round(r.commission || 0)) ? 1 : 0, // возврат OZON обнуляет комиссию сам - это не пропуск
        ...
        cc: Math.round((cogs[sk] || 0) * units * (1 - (r.revenue > 0 ? Math.min(1, retAmt / r.revenue) : 0))), noCs: (units > 0 && cogs[sk] == null),
      });
    }
...
function paint(p,src,ordGrand){
  ...
  // Страж свежести водопада: ряд P&L канала заморожен на AN_MAXD (transaction/list отключён OZON
  // 08.09.2026). Если выбранный период уходит дальше AN_MAXD - данных за хвост нет, честно помечаем,
  // иначе водопад молча показывал бы неполный месяц как полный.
  var pnlStale=''; // водопад OZON строится на своде по заказам (по дате заказа, свежий), заморозка канального P&L транзакций больше не влияет
  const badge='<span class="kt-src">'+(p.daily?'за период ':'снимок ')+p.dateFrom+'..'+p.dateTo+'</span>'+note+pnlStale;
  ...
  if(!${IS_OZON}){
    ...
    if(t&&!t.empty){
      ...
      if(Math.round(t.shipCov||0))wfSteps.push(['Наша доставка (счёт перевозчика, ведомость)'
        +(Math.round(t.shipLost||0)?'. Внутри - '+svRub(t.shipLost)+' ₽ по '+svZakOtm(t.shipLostN)+': перевозку оплатили, выручки нет':''),
        -t.shipCov,'#F59E0B','Наша доставка']);
      // Два бара вместо одного (Катя 23.09.2026: «водопад тоже корректируй»). Пока АДМ и налог
      // стояли на одной базе, общий бар читался как 45% от поступления и проверялся в уме. С
      // 23.09.2026 базы разные: АДМ от поступления, налог от платежа покупателя, - и склеенный
      // бар стал числом, которое неоткуда вывести. Разделение возвращает проверяемость: у каждого
      // бара своя база названа в подсказке. Цепочка от этого не меняется, сумма та же.
      var _ae=document.getElementById('sv-adm'),_te=document.getElementById('sv-tax');
      var _ar=Number((_ae&&_ae.value)||30),_tr=Number((_te&&_te.value)||15);
      if(Math.round(t.adm))wfSteps.push(['АДМ '+_ar+'% от поступления ('+svRub(t.T.net)+' ₽)',
        -t.adm,'#F59E0B','АДМ '+_ar+'%']);
      if(Math.round(t.tax))wfSteps.push(['Налоги '+_tr+'% от того, что оплатил клиент ('+svRub(t.T.paid||0)+' ₽). Это НЕ поступление: поступление уже за вычетом сборов Маркета, а налог на них не уменьшается',
        -t.tax,'#F59E0B','Налоги '+_tr+'%']);
  ...
  if(!wfSteps&&ordGrand){
    ...
    steps2.push(['К выплате',g.amt||0,'#34D399','К выплате']);
  ...
  if(!wfSteps){
    // Фолбэк (нет свода по заказам за период): прежняя цепочка из аналитики по SKU.
    var pt=periodTotals(p.dateFrom,p.dateTo);
    var feeTot2=Math.round(pt.amt-pt.accr);var F=pt.fees||{};var fmtF2=function(v){return fmtRu(Math.round(v));};
    var feeTip2='Всего сборов: комиссия '+fmtF2(F.comm)+', логистика '+fmtF2(F.log)
      +(Math.round(F.acq)?', эквайринг '+fmtF2(-F.acq):'')+(Math.round(F.sto)?', хранение '+fmtF2(-F.sto):'')
      +(Math.round(F.partner)?', услуги партнёров '+fmtF2(F.partner):'')+', прочие '+fmtF2(F.proch)+', реклама '+fmtF2(F.ads);
    var steps3=[['Начислено',pt.accr,'#22D3EE','Начислено']];
    if(feeTot2)steps3.push([feeTip2,feeTot2,'#FF5A5F','Сборы']);
    steps3.push(['К выплате',pt.amt,'#34D399','К выплате']);
...
// База АДМ по артикулу (Иван 25.09.2026, п. 2.4, вариант Б): «К выплате» артикула считается ПО ДНЯМ,
// и отрицательный день даёт ноль - расходы дня без продаж АДМ не уменьшают. Раньше в базу шёл
// «К выплате» целиком у артикулов с продажами в окне, и расход, стоящий в одной половине месяца,
// попадал в базу только при выборе всего месяца: части не складывались (сентябрь, 46 235 ₽).
// Сумма по дням с отсечкой складывается по любым периодам. Состав дня - тот же, что у «К выплате»
// строки: финансы по SKU за вычетом их эквайринга и хранения, минус эквайринг и хранение by-day,
// минус реклама CPC и CPO.
function admBase(sk,from,to){
  var d={},add=function(rows,idx,sgn){if(!rows)return;for(var i=0;i<rows.length;i++){var r=rows[i];if(r[0]<from||r[0]>to)continue;d[r[0]]=(d[r[0]]||0)+sgn*(r[idx]||0);}};
...
// День покрыт отчётом о реализации за день (OZON) - внутри диапазона, который отчёт отдал.
function inRd(d){return AN_RD_FROM&&d>=AN_RD_FROM&&d<=AN_RD_TO;}
function realUnits(sk,covM,from,to){
  ...
  // Дни вне закрытых месяцев: отчёт о реализации за день, где он есть (тот же базис, что деньги
  // таблицы с FIN_CUT), иначе дневной ряд доставок (доставлено − возвраты).
  var rd=AN_RDAY[sk]||[];
  for(var k=0;k<rd.length;k++){var q=rd[k];if(q[0]<from||q[0]>to)continue;if(covM[q[0].slice(0,7)])continue;u+=q[1]||0;}
  var sd=AN_SALES[sk]||[];
  for(var j=0;j<sd.length;j++){var d=sd[j];if(d[0]<from||d[0]>to)continue;if(covM[d[0].slice(0,7)])continue;if(inRd(d[0]))continue;u+=(d[3]||0)-(d[4]||0);}
  return u;
}
// База налога по SKU за период (Иван 25.09.2026): закрытый отчётом месяц целиком в окне - «Реализовано»
...
function realBase(sk,covM,from,to){
  ...
  for(var i=0;i<rr.length;i++){if(cov[rr[i][0]]&&rr[i][3]!=null)b+=rr[i][3];}
...
function anCells(x){
  ...
  // Налог - 15% от «Реализовано» (x.tb, база налога из отчёта о реализации), если строка её несёт;
  ...
  // «В пути» - тоже только в блоке по заказам: штуки и выручка заказов, которые ещё не доставлены
  // (x.fly определён только там). Янтарным, как статус летящего заказа в строке.
  var FLY=function(v){return '<td class="r"'+(v?' style="color:#E5B567"':'')+'>'+(v?fmtRu(Math.round(v)):'—')+'</td>';};
...
function renderSkuAnalytics(cur){
  ...
  for(var sk in AN_META){
    ...
    // adsk = CPC-реклама по SKU (per-SKU спенд). Включаем в guard: SKU с рекламным расходом, но БЕЗ продаж
    // (ad[0]=AN_ADS и cpo его не ловят) таблица иначе пропускала, а periodTotals (план) - учитывал, и
    // «Чистая прибыль» плана и таблицы расходилась на 0.45×спенд (август ~10 920). Теперь оба видят SKU.
    var adsk=anSum(AN_ADSSKU[sk],from,to,1)[0]||0;
    if(!sa[0]&&!sa[1]&&!sa[2]&&!sa[3]&&!sa[4]&&!ad[0]&&!fi[0]&&!fi[6]&&!cpo&&!ship&&!dinc&&!acq&&!sto&&!adsk)continue;
    var m=AN_META[sk];
...
function renderOrdersAnalytics(cur){
  ...
  // берём из per-SKU рядов таблицы по артикулам и раскидываем по заказам этого артикула (вес -
  // реализованные штуки; остаток - на последний заказ, без потери копеек):
  ...
  var spread=function(arr,wsum,amount,key,cutAmt){amount=Math.round(amount||0);if(!amount)return;var acc=0;
    for(var j=0;j<arr.length;j++){var o=arr[j];var w=(o.units>0?o.units:0);
      ...
      o[key]=(o[key]||0)+part;if(cutAmt){o.amt=(o.amt||0)-part;if(o.amtS)o.amtS=(o.amtS||0)-part;}}
  };
  ...
  // «Доставка покупателя» (доход) - PER-ORDER из accrual/by-day по ключу заказа (AN_BDORD). Пропорцию НЕ
  // используем: несматченный кабинетный остаток - это заказы ВНЕ окна (оформлены до начала периода сбора,
  ...
  var SUMK=['units','dlv','fly','flyAcc','tb','tbFly','estFee','eCom','eDel','eOth','ePrt','nNoFee','acc','com','del','acq','sto','oth','prt','adv','amt','amtS','cc','ship','dinc'];
  ...
  var grand={};SUMK.forEach(function(k){grand[k]=0;});var html='';
  cats.forEach(function(g){SUMK.forEach(function(k){grand[k]+=g.t[k]||0;});var op=!!anOpenOrd[g.cat];var ck=g.cat.replace(/"/g,'');
    // Тип доставки известен только у доставленных: rFBS выставляется по realFBS-начислению, а оно
  ...
  // Без этой строки ИТОГО в разрезе заказа не сходилось бы с таблицей по артикулам на сумму кабинетных
  ...
  if(estEl){var eTx=[];
    ...
    if(grand.nNoFee)eTx.push('<span class="an-nofee">Доставленных заказов без начисленной комиссии: '+grand.nNoFee+'</span> (возвраты не в счёт - по ним комиссию обнуляет сам OZON). Строки помечены «доставлен, комиссия не начислена», ячейка комиссии - оранжевая с пунктиром: «К выплате» по ним может быть завышен на размер комиссии.');
...
// === блок «Аналитика по артикулам (по дате заказа)» (Иван 25.09.2026) ===
// Строки - те же заказы, что в таблице по заказам, ПОСЛЕ всех доборов (реклама CPC, эквайринг,
// хранение, партнёры, доставка): складываем их по артикулу, а не считаем заново. Поэтому ИТОГО
// обязан совпасть с ИТОГО таблицы по заказам по каждой колонке - это и есть проверка блока.
var anOpenOrdSku={};
function renderOrdSku(rows,SUMK,acctRow,grand){
  var el=document.getElementById('ordsku'),tb=document.getElementById('ordsku-t');if(!el||!tb)return;
  var src=document.getElementById('ordan-t');
  ...
  var cats=Object.keys(groups).map(function(c){var arr=groups[c];var t={n:0,sch:{}};SUMK.forEach(function(f){t[f]=0;});
    ...
    arr.sort(function(a,b){return b.acc-a.acc;});return {cat:c,arr:arr,t:t};}).sort(function(a,b){return b.t.acc-a.t.acc;});
  if(!cats.length){el.innerHTML='<tr><td colspan="26" class="kt-note">нет заказов за период</td></tr>';return;}
  var html='';
  cats.forEach(function(g){var op=!!anOpenOrdSku[g.cat];var ck=g.cat.replace(/"/g,'');
    html+='<tr class="ord-cat" data-cat="'+ck+'"><td>'+(op?'▾ ':'▸ ')+g.cat+' <span style="color:var(--ink-3);font-weight:400">('+g.arr.length+' арт., '+g.t.n+' зак.)</span></td><td style="color:var(--ink-3)">'+schOf(g.t.sch)+'</td>'+anCells(g.t)+'</tr>';
    g.arr.forEach(function(a){html+='<tr class="ord-row" data-cat="'+ck+'" style="'+(op?'':'display:none')+'"><td title="'+String(a.nm||'').replace(/"/g,'&quot;')+'">'+(a.off||a.sk)+' <span style="color:var(--ink-3);font-weight:400">'+a.n+' зак.</span></td><td>'+(schOf(a.sch)||'—')+'</td>'+anCells(a)+'</tr>';});
  });
  if(acctRow)html+='<tr style="cursor:default;font-weight:600" title="Та же строка, что в таблице по заказам: сборы кабинета без артикула и доставка по заказам, чей артикул не сошёлся с каталогом."><td>Общие расходы</td><td></td>'+anCells(acctRow)+'</tr>';
  el.innerHTML='<tr style="font-weight:800;background:rgba(34,211,238,.16);border-top:2px solid #22D3EE;border-bottom:2px solid #22D3EE"><td style="color:#22D3EE">ИТОГО</td><td></td>'+anCells(grand)+'</tr>'+html;
  el.querySelectorAll('.ord-cat').forEach(function(tr){tr.onclick=function(){var c=tr.getAttribute('data-cat');anOpenOrdSku[c]=!anOpenOrdSku[c];var td=tr.querySelector('td');td.innerHTML=td.innerHTML.replace(anOpenOrdSku[c]?'▸':'▾',anOpenOrdSku[c]?'▾':'▸');el.querySelectorAll('.ord-row[data-cat="'+(window.CSS&&CSS.escape?CSS.escape(c):c)+'"]').forEach(function(s){s.style.display=anOpenOrdSku[c]?'':'none';});};});
}
...
function renderAccountFees(cur){
  ...
  // теперь на стороне артикулов (dinc), остаток - здесь; вместе = кабинетная доставка = P&L канала.
  var distDinc=0;for(var _s in AN_DINCSKU){distDinc+=anSum(AN_DINCSKU[_s],from,to,1)[0]||0;}
  f.delivery=Math.round(f.delivery-distDinc);
...
// Факт за месяц (те же формулы и ИСТОЧНИКИ, что ИТОГО таблицы): К выплате после разнесённой рекламы,
// эквайринг/хранение из accrual/by-day, наша доставка/доход покупателя из AN_SHIPSKU/AN_DINCSKU
// (delivery_orders, по заказу) - всё как в таблице. За ЗАКРЫТЫЙ месяц «Чистая прибыль» плана и ИТОГО
// таблицы совпадают. За НЕЗАКРЫТЫЙ отличаются на висящие заказы: план на реализованной базе (amtReal,
// без ещё не выкупленных), таблица - по начислениям (amt). Это единственная разница базы, она подписана
// в карточке плана; источники доставки/эквайринга/К выплате едины.
function periodTotals(from,to){
  ...
  for(var sk in AN_META){var sa=anSum(AN_SALES[sk],from,to,5),fi=anSum(AN_FIN[sk],from,to,7);var ru=realUnits(sk,covM,from,to);
    ...
    rev+=sa[0]||0;accr+=(fi[0]||0);realized+=ru;amt+=amtNet;if(${IS_OZON}){amtReal+=amtNet;amtS+=admBase(sk,from,to);}else{amtReal+=(ru>0?amtNet:Math.min(0,amtNet));if(ru>0)amtS+=amtNet;}cc+=(AN_COGS[sk]||0)*ru;gadv+=adv;if(${IS_OZON})tb+=realBase(sk,covM,from,to);
  ...
  // кабинета АДМ не уменьшают. Маркет - прежние правила.
  amt+=at;amtReal+=at;amtS+=${IS_OZON}?Math.max(0,at):at;
  // «Наша доставка» (наш расход на отправку, ПЭК/СДЭК) и «Доход с покупателя» за период - тот же
  // источник и та же привязка (по заказу, все месяцы), что ИТОГО таблицы: AN_SHIPSKU/AN_DINCSKU
  // (из delivery_orders). Раньше здесь была ведомость по offer (AN_DELIV, по дате ОТГРУЗКИ, закрытые
  ...
  if(!${IS_OZON})tb=amtS;
  var net=(amtReal-cc-ship+dinc)-0.30*amtS-0.15*tb,rent=(amtReal>0)?net/amtReal*100:null; // прибыль/рентаб - на реализованной базе (без висящих заказов)
  ...
  return {ad:-aB.adv,rev:rev,accr:accr,realized:realized,amt:amt,amtReal:amtReal,amtS:amtS,tb:tb,cc:cc,ship:ship,dinc:dinc,delivery:aB.delivery,net:net,rent:rent,fees:fees};
}
...
// Верхний горизонтальный скроллбар: тонкая полоса .an-htop над таблицей (под подписями столбцов
// сверху), синхронизированная с прокруткой .an-vscroll - чтобы не тянуться к ползунку в самый низ.
function syncTopScroll(){
  var tops=document.querySelectorAll('.an-htop');
  for(var i=0;i<tops.length;i++){(function(top){
    var bot=top.nextElementSibling;if(!bot||!bot.querySelector)return;var tbl=bot.querySelector('table');if(!tbl)return;
    top.firstChild.style.width=tbl.scrollWidth+'px';
    top.style.display=(tbl.scrollWidth>bot.clientWidth+1)?'block':'none'; // прячем, если скроллить нечего
    if(!top._wired){top._wired=1;var lock=false;
...
function svAgg(ms,w,lost,ohPer){
  ...
  var a={};ms.forEach(function(m){m.rows.forEach(function(r){if(!svInWin(r.d,w))return;var k=r.sku,o=a[k];
    ...
    // Сколько строка стоила ПОКУПАТЕЛЮ. С 23.09.2026 это база налога, поэтому хранится отдельным
    // полем, а не берётся из rev: rev - историческое поле сортировки, и смешивать их нельзя.
...
// Отправки по заказам, которые отменили или вернули. Перевозку мы оплатили, выручки нет.
// В строки артикулов такой расход не идёт (рентабельность артикула поехала бы от расхода без
// продажи) - он стоит своей строкой, решение Ивана 18.09.2026. По дням, чтобы работали окна
// короче месяца.
...
function svDraw(){
  ...
  (function(){
    ...
    gaps.push('перевозка по '+svZakOtm(n)+' на '+svRub(v)+' ₽ разнесена на '+svArt(ls.length)
...
function svCalcAll(list,ohM,ohP,adm,tax){
  ...
  //   Валовая = Поступление − СС;  АДМ − от Поступления;  Чистая = Валовая − АДМ − Налоги
  // НАЛОГ с 23.09.2026 считается от ПЛАТЕЖА ПОКУПАТЕЛЯ, а не от поступления (Катя: «перестрой
  // расчет налогов - 15% от суммы оплатил клиент»). Базы разные нарочно: поступление - это уже
  // за вычетом сборов Маркета, а налог на них не уменьшается.
  ...
  function calc(a){
    ...
    // Вычитается ПОСЛЕ поступления, как и С\С: это не сбор площадки, поэтому ни базу АДМ
  ...
  groups.forEach(function(g){
    ...
    T.shipOur+=g.shipOur||0;T.shipKn=T.shipKn||!!g.shipKn;T.shipCov+=g.shipCov||0;});
...
function soAgg(ms,w,lost,ohPer){
  ...
  ms.forEach(function(m){(m.rows||[]).forEach(function(r){
    ...
    // Сколько заказ СТОИЛ ПОКУПАТЕЛЮ. Не наша выручка и не часть расчёта: в «Продажах» стоит прайс
    // целиком, вместе с долей Маркета, которую он платит за покупателя и возвращает нам баллами.
    // За июль прайс 9.86 млн, а покупатели отдали 5.07 млн - разницу в 4.79 млн внёс Маркет своей
...
function soDraw(){
  ...
  // «Оплатил клиент» - СПРАВОЧНАЯ колонка (Катя 23.09.2026). Стоит сразу после «Продаж», потому
  // что смысл у неё ровно в сравнении с ними, а через восемнадцать колонок это сравнение не
  // делается. В цепочку «Продажи + Доставка покупателя − сборы = Поступление» она НЕ входит и в
  // ИТОГО ничего не добавляет; доставка покупателя внутри неё уже сидит, складывать их нельзя.
  ...
  // Приглушена нарочно: колонка справочная, и в денежной цепочке её быть не должно даже глазами.
  function paidCell(x){
    var v=x.paid||0;
    return '<td class="r" style="color:var(--ink-3)"'+(Math.round(v)&&Math.round(x.priceNet)
  ...
  // Общие расходы кабинета разносятся по штукам. Если штук в окне нет - разносить не на что, а
  // расход есть: подписка, полки и буст списываются по календарю, а не по продажам. Катя
  // 23.09.2026: «пока нет заказов и раскидывать некуда - клади в общие расходы». До этого такой
  // остаток попадал только в колонку «Прочее», а поступление и чистая оставались нулями, и строка
  // одновременно утверждала «потратили 9 334 ₽» и «прибыль ноль». Теперь он вычитается: за
  // 18-21.09 это поступление -9 334 и чистая -9 334. Когда штуки есть, остаток равен нулю
...
var SV_TAX_TIP='База - колонка «Оплатил клиент», а не «Поступление». Поступление - это уже за вычетом сборов Маркета, а налог на них не уменьшается. АДМ по-прежнему берётся от поступления: базы разные нарочно. Решение Кати от 23.09.2026, подтверждено Иваном; источник ставки и базы - knowledge/semantic/ym-rates-adm-tax.md. Режим налогообложения там же помечен как неподтверждённый бухгалтерией: число для управленческого учёта, не для отчётности.';
...
var SV_ADM_TIP='Считается от «Поступления» - базу ему не меняли. На строке с ОТРИЦАТЕЛЬНЫМ поступлением не начисляется, поэтому ИТОГО равно сумме строк, а не ставке от сложенного поступления: минус одной строки не должен гасить расход соседних. Так же в раскладке Ивана и в блоке OZON. Источник: knowledge/semantic/ym-rates-adm-tax.md.';
...
// «по 1 заказу, которые отменили» - та же мелочь, что и «по 1 заказам»: число рядом перестаёт
// читаться как посчитанное. Одним хелпером, чтобы согласование не разъехалось между блоками.
function svZakOtm(n){var a=n%10,b=n%100;return svZak(n)+', '+((a===1&&b!==11)?'который':'которые')+' отменили или вернули';}
...
function svTabPnl(list,ohM,ohP,noteEl){
  ...
  // «Оплатил клиент» - та же справочная колонка, что в своде по заказам, и по той же причине
  // стоит сразу после «Продаж». С 23.09.2026 она перестала быть только справкой: от неё считается
  // налог, и процент от числа, которого нет на экране, проверить было бы нечем.
  var H=['Категория / Артикул','Продажи','Оплатил клиент','Доставка покупателя'].concat(FEE)
    .concat(['Баллы Маркета','Штуки','Поступление','Наша доставка','С\\С произв.',
             'Валовая прибыль','Маржа','АДМ '+svPct(adm),'Налоги '+svPct(tax),'Чистая прибыль','Рентаб.',
  ...
  // Приглушена: в цепочке «Продажи + Доставка покупателя − сборы = Поступление» её нет. Но это уже
  // не просто справка - от неё считается налог, поэтому прочерк ставим только при настоящем нуле.
  function svPaidCell(a){
    var v=a.paid||0;
    return '<td class="r" style="color:var(--ink-3)"'+(Math.round(v)&&Math.round(a.priceNet||0)
      ? ' title="покупатель отдал '+svRub(v)+' ₽ при прайсе '+svRub(a.priceNet)+' ₽ - разницу '+svRub((a.priceNet||0)-v)+' ₽ внёс Маркет скидкой и вернул нам баллами. Налог в строке - '+svPct(tax)+' от '+svRub(v)+' ₽"' : '')
      +'>'+(Math.round(v)?svRub(v):'—')+'</td>';
  }
