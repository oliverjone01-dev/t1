...
## РЕШЕНИЕ ИВАНА (2026-09-16) - ФИНАЛЬНАЯ БАЗА, зафиксировано
После сверки трёх формул на августе по ФАКТИЧЕСКОЙ цене каждого SKU (1008 арт.):
- **«Начислено» = доставлено − возвраты** (по цене SKU) = 18,48 млн vs старое 18,90 млн -> **Δ −2.2%** (лучший сход).
- «продажи − возвраты − отмены» = 20,51 млн (+8.5%) - отвергнуто (захватывает недоставленное; старое
  «начислено» OZON начислял в момент ДОСТАВКИ, поэтому доставлено−возвраты = старая база).
- Остаток −2.2% = цена витрины×доставлено против фактической суммы начисления OZON (округления/часть
  скидок), в пределах допуска §15.

ИТОГОВАЯ ФОРМУЛА МИГРАЦИИ (использовать в пересборке pnl-*):
1. **Начислено (выручка к учёту) = Σ по SKU (delivered − returns) × цена_SKU** из history.ndjson
   (свежий ряд продаж; цена_SKU = revenue/units этого SKU за период). По штукам совпадает с УПД (296≈299).
2. **Сборы = accrual/postings (комиссия+логистика продаж) + accrual/by-day (эквайринг/хранение/прочее)** -
   проверены до рубля (комиссия Δ5₽ на 9 млн).
3. **К выплате = Начислено − Сборы.**
4. Закрытый месяц можно дополнительно сверять с реализацией (УПД) как контроль, но БАЗА дашборда -
   доставлено−возвраты (не подменять на нетто-УПД 8,4 млн - решение Ивана: держим «как в старом»).
