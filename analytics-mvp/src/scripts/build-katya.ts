...
// --- страница 5: Деньги (ЖИВЫЕ P&L-вебхуки по периоду, fallback - снимок) ---
{
  ...
  try {
    for (const l of readFileSync(dp("orders_daily.ndjson"), "utf-8").trim().split("\n").filter(Boolean)) {
      ...
      if (st === "cancelled") continue; // минус отмены
...
`;
  ...
  ${IS_OZON ? `<section class="card"><div class="card-h"><div><div class="card-title">Аналитика по артикулам (по дате заказа)</div><div class="card-sub">Те же заказы и те же числа, что в таблице по заказам выше, только сложенные <b>по артикулу</b>: в период попадают <b>заказы, оформленные в выбранные даты</b>, а не начисления за эти даты, как в таблице по артикулам ниже. Отменённые не входят; доставленные и ещё летящие - входят, летящие видны в колонках «В пути». ИТОГО совпадает с ИТОГО таблицы по заказам. Клик по категории раскрывает артикулы.</div></div></div><div class="an-htop"><div></div></div><div class="kt-scroll an-vscroll"><table class="kt-table" id="ordsku-t"><thead></thead><tbody id="ordsku"></tbody></table></div></section>` : ``}
