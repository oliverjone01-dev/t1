...
function renderSkuAnalytics(cur){
  ...
  // Сборы уровня заказа/кабинета (реклама/штрафы/realFBS/доставка от покупателя и т.п.) в ИТОГО
  // НЕ включаем: по решению Ивана они компенсируются и в расчёт по SKU не входят. Детальная
  // разбивка остаётся отдельным информационным блоком ниже.
