...
function paint(p,src){
  ...
  var pnlStale=(${IS_OZON}&&AN_MAXD&&p.dateTo>AN_MAXD)?(' <span style="color:#E5B567" title="Ряд P&L канала заморожен: метод OZON transaction/list отключён 08.09.2026. За '+ (function(){var a=AN_MAXD.split("-").reverse().join("."); return a;})() +'..'+p.dateTo.split("-").reverse().join(".")+' данных нет.">⚠ P&L только по '+AN_MAXD+'</span>'):'';
...
function renderOrdersAnalytics(cur){
  ...
  // PER-ORDER где ключ заказа сошёлся (AN_BDORD, ~85%); несматченный остаток (кабинетный итог периода −
  // Σ сматченного) разносим по rFBS-заказам без матча пропорц. штукам. ИТОГО = кабинетный ряд за период.
  var bdTotal=Math.round(anSum(AN_BUYERDELIV,from,to,1)[0]||0);
  var bdUsed={},bdMatched=0,bdRest=[];
  for(var r2=0;r2<rows.length;r2++){var o2=rows[r2];var ob=(o2.order||'').replace(/-\d+$/,'');
    if(AN_BDORD[ob]!=null&&!bdUsed[ob]){bdUsed[ob]=1;var v=Math.round(AN_BDORD[ob]);o2.dinc=(o2.dinc||0)+v;bdMatched+=v;}
    else if(o2.scheme==='rFBS'){bdRest.push(o2);}}
  var bdRem=bdTotal-bdMatched;
  if(bdRem>0&&bdRest.length){var uR=0;for(var r3=0;r3<bdRest.length;r3++)uR+=(bdRest[r3].units>0?bdRest[r3].units:0);var acR=0;
    for(var r4=0;r4<bdRest.length;r4++){var o3=bdRest[r4];var w3=(o3.units>0?o3.units:0);
      var pR=(r4===bdRest.length-1)?(bdRem-acR):(uR>0?Math.round(bdRem*w3/uR):Math.round(bdRem/bdRest.length));acR+=pR;o3.dinc=(o3.dinc||0)+pR;}}
  ...
  cats.forEach(function(g){SUMK.forEach(function(k){grand[k]+=g.t[k]||0;});var op=!!anOpenOrd[g.cat];var ck=g.cat.replace(/"/g,'');
    var schCat=(function(){var s={};g.arr.forEach(function(x){if(x.scheme)s[x.scheme]=(s[x.scheme]||0)+1;});return Object.keys(s).sort(function(a,b){return s[b]-s[a];}).join('/');})();
    ...
    g.arr.forEach(function(x){var stb=(x.st&&x.st!=='delivered')?' <span style="color:#E5B567">'+x.st+'</span>':'';var lbl=(x.off||x.order)+' <span style="color:var(--ink-3);font-weight:400">'+x.order+'</span>'+stb;html+='<tr class="ord-row" data-cat="'+ck+'" style="'+(op?'':'display:none')+'"><td title="'+String(x.nm||'').replace(/"/g,'&quot;')+'">'+lbl+'</td><td>'+(x.scheme||'—')+'</td>'+anCells(x)+'</tr>';});
  });
...
function render(cur,cmp){
  ...
  if(${IS_OZON})renderOrdersAnalytics(cur); // аналитика по заказам (в разрезе заказа)
