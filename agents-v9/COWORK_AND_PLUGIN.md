...
```
Тот же плагин доступен из лёгкой ветки `plugin` (`/plugin marketplace add oliverjone01-dev/t1@plugin`; CLI принимает и `#plugin`, сохраняет как `@plugin`): содержимое
...
Практический путь:
1. Desktop → боковая панель **Customize** → Plugins → **Add marketplace** → строка `oliverjone01-dev/t1@plugin` → Sync →
   установить `gengroup-roster`.
   Почему с суффиксом `@plugin` [ДАННЫЕ 2026-09-10]: `claude plugin marketplace add oliverjone01-dev/t1@plugin` клонирует
   только ref `plugin` за 2.0 с, `claude plugin install gengroup-roster@gengroup` кладёт 880 КБ в
   `~/.claude/plugins/cache/gengroup/gengroup-roster/3.0.3`, `claude plugin list` показывает версию 3.0.3 enabled.
   Полный main (465 МБ по GitHub API: аналитика, xlsx, PDF в истории) при этой форме не выкачивается.
   Формы, которые Desktop отверг [ДАННЫЕ 2026-09-08]: `oliverjone01-dev/t1#plugin` - «Marketplace source format is invalid.
   Enter a GitHub owner/repo or a full Git URL», то есть разделитель `#` поле Desktop не разбирает;
   `https://github.com/oliverjone01-dev/t1.git#plugin` и `oliverjone01-dev/t1` без ветки - «Marketplace sync failed»,
   клон ветки по умолчанию не укладывается в лимит.
   Форма `oliverjone01-dev/t1@plugin` именно в поле Desktop [ГИПОТЕЗА]: в CLI проходит, в Desktop не проверялась,
   проверяет Иван. Ветку `plugin` обновляет `bash .claude-plugin/export-plugin.sh --push` после любого изменения
   плагина в main (руками её не править).
   План Б, если Desktop не примет `@`: отдельный репозиторий-зеркало `oliverjone01-dev/gengroup-roster` (~600 КБ,
   `.claude-plugin/marketplace.json` в корне с `source: ./plugins/gengroup-roster`, плагин в `plugins/gengroup-roster/`,
   README), сборка и заливка `bash .claude-plugin/export-plugin.sh --repo https://github.com/oliverjone01-dev/gengroup-roster.git --push`,
   раскладка проверена локально через CLI: Skills 19, Agents 13, Hooks 5. Репозиторий создаёт Иван (New repository,
   Public, без README): интеграции Claude создание запрещено, `POST /user/repos` отдаёт 403 «Resource not accessible by
   integration» (проверено 2026-09-08 и 2026-09-10).
   [ДАННЫЕ 2026-09-07] сторонний маркетплейс в Customize добавлять можно, поле принимает `owner/repo` или git URL,
   суффикс ветки задокументирован в docs discover-plugins.
...
| B | Desktop → Customize → Plugins → Add marketplace | Строка `oliverjone01-dev/t1@plugin`, затем установить `gengroup-roster`: Cowork и облачные сессии получают `<name>@synced` (13 агентов, 19 skills, 5 хуков). Форма с `@` проверена в CLI (клон только ref `plugin`, 2.0 с, 880 КБ), в поле Desktop - гипотеза. Не проходят: `oliverjone01-dev/t1` и `t1.git#plugin` (sync failed, клон ветки по умолчанию), `t1#plugin` (invalid format). План Б - зеркало `oliverjone01-dev/gengroup-roster`, создаёт Иван (§3). |
