...
- **Жёсткие правила в system prompt.** Метки `[ДАННЫЕ]`/`[ГИПОТЕЗА]`, запрет на длинное тире, флаг VALONTI как нарушение (VIOLUR - легальные столы GENGLASS, см. DOC_05 §1.6), фиксированный формат ответа (Вердикт, Что произошло, Разбор, Риски, Задачи P1-P3 с ответственными).
