...
> **Примечание 2026-09-06 (аудит ростера v3, gap 6).** В этом отчёте три значения одной оценки: 8.15 (JSON и заголовок),
> 8.350 (собственный блок расчёта ниже) и 9.35 (Σ вектора 10/8/10/10/9 по весам 0.25/0.25/0.20/0.15/0.15).
> Оценка задним числом не пересчитывается; якорь в `.claude/skills/phoenix-eval/references/calibration-anchors.md` помечен
> [РАСХОЖДЕНИЕ В ИСТОЧНИКЕ] и используется только как класс вердикта. Решение о пересчёте - за Иваном. С v3 такой отчёт
> не проходит `schemas/validate.py` (допуск 0.05).
>
> **Решение Ивана 2026-09-08.** Якорь в calibration-anchors.md пересчитан на 9.35 по контракту v3 (weighted_total = Σ);
> этот отчёт не редактируется, его 8.15 остаётся историческим значением.
