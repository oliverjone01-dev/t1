...
export function DailySpend() {
  ...
  return (
    <Frame title="Дневной расход до и после остановки Товарной галереи"
      note="Товарная галерея остановлена 16 августа. Пунктиром показаны средние по каждому периоду. Дневной расход после остановки не упал: по среднему он даже вырос на 13,5%, но по медиане остался прежним (5 376 против 5 348 ₽), потому что в период «до» попали три слабых дня. Устойчивый вывод один: экономии нет. Переток на Поиск устойчив по обоим способам счёта, +37% по среднему и +20% по медиане: у кампаний задан дневной бюджет, а не общий потолок."
      h={H}
    >
...
export function VisibilityGap() {
  ...
  return (
    <Frame title="Заметность перегородочных страниц: мы и loftcase.ru" h={H}
      note="Тридцать три наши страницы дают 183 единицы, тридцать их страниц - 1 042. Причём 944 из этих 1 042 приносят всего две страницы: /partition и /peregorodki."
    >
      <svg viewBox={`0 0 ${W} ${H}`} role="img" aria-label="Сравнение заметности с конкурентом">
        ...
        <text className="ax fade" x={pad} y={H - 6} fill="var(--ink-3)">серым выделены две страницы loftcase, дающие 944 из 1 042</text>
      </svg>
    </Frame>
  );
}
