...
Разметка уже стоит на странице и рассчитана на эти имена файлов. Пока файла нет, блок скрывается сам и на вёрстку не влияет: класть готовые файлы в `public/img/` можно по одному, пересборка не требуется.
