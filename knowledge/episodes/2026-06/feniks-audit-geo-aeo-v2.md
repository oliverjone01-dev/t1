...
| B5 | Priority queries по 5 брендам actionable | 8 | Каждый бренд 3-5 запросов, GLASS-MEMORY → 5 |
...
| C4 | Niche priority logic обоснована | 7 | Q2 GLASS-MEMORY (untaken) → Q3-Q4 GENGLASS pillar → 2027 VALONTI. Logical но без [ДАННЫЕ] подтверждения «GLASS-MEMORY untaken». Допущение, должно быть `[ГИПОТЕЗА]` |
| C5 | Comparative статement examples non-trivial | 7 | «visual continuity при зонировании» - good, но всего 1 пример. Маловато |
...
| E2 | P9 Hard Rules не нарушены | 8 | Нет «удвоит», «выстрелит». Но «80%+ Share of Model получит первый» в v8 source было розовыми очками - **в v9 справедливо убрано** ✅ |
...
v8 имел в 7-point checklist пункт #6 **«Long-Tail Coverage»** (текст отвечает на 3+ длинных запроса, каждый = отдельный абзац/FAQ-пара). v9 заменил его на **Source Attribution** (именные эксперты).
...
**Risk:** Skill подаётся как «выполни 7 пунктов = AI Citation Rate ↑». Если результат не материализуется, нет escalation path / debug guidance.
...
```json
{
  ...
  "rework_tz": [
    "1. Добавить пункт 8 'Long-Tail Coverage' (3+ запроса 5+ слов = отдельные FAQ-пары) ИЛИ задокументировать removal с rationale",
