...
function paint(a,src){
  ...
  document.getElementById('top').innerHTML=(a.top_spend||[]).map(function(c,ci){
    ...
    if(has){
      ...
      if(omM>0||soM>0){rowsH+='<tr><td style="color:var(--ink-3)">Объединённая карточка (др. SKU)</td><td></td><td></td><td class="r">—</td><td class="r">'+fmtRu(omM)+'</td><td class="r">'+soM+'</td><td class="r">—</td></tr>';}
      var inner='<div style="overflow-x:auto"><table class="kt-table" style="margin:0;min-width:520px"><tbody>'+rowsH+'</tbody></table></div>';
