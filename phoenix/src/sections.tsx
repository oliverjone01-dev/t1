    ...
    <header className="hero">
      ...
      <Reveal delay={180}>
        <p className="hero__sub">
          За 30 дней августа направление съело всю рекламу компании и принесло 95 заявок.
...
/* ================= словарь ================= */
export function Slovar() {
  const terms: Array<[string, string]> = [
    ["Органика, выдача", "бесплатные результаты поиска, не реклама"],
    ["В индексе", "Яндекс нашёл страницу и готов показывать её по запросам"],
    ["Топ-3, топ-10, топ-50", "по скольким запросам страница стоит на первых трёх местах, на первой странице, в первых пяти страницах"],
    ["Заметность", "оценка keys.so, насколько сайт или страница видны в поиске. Внутренняя шкала сервиса: в посетителей и рубли не переводится. Шкала для сайта целиком и для отдельной страницы разные, между собой их не сравнивают"],
    ["Авторитет", "оценка keys.so по числу и качеству ссылок на сайт со стороны, шкала 0-100"],
    ["Хаб", "главная страница раздела, куда ведут все подстраницы этого типа"],
    ["Склейка, 301-редирект", "переадресация со старого адреса на новый, чтобы поисковик считал их одной страницей и не делил доверие пополам"],
    ["Доля обращений", "какая доля посетителей оставила заявку. В тексте встречается ещё как «доля визитов с заявкой» и «конвертирует» - это одно и то же"],
    ["Кликабельность, CTR", "какая доля тех, кому показали объявление, по нему кликнула"],
    ["Цена клика", "сколько платим за один переход из рекламы"],
    ["Заявка", "обращение с сайта: форма, звонок, сообщение. Считается целью Метрики «CRM. Все лиды»"],
    ["Цена заявки", "сколько рублей рекламы приходится на одно обращение"],
    ["ВЧ, СЧ, НЧ", "высоко-, средне- и низкочастотные. Так названы три наши поисковые кампании по типу запроса: ВЧ короткие популярные («перегородки»), СЧ средние («перегородки для зонирования»), НЧ редкие подробные («перегородка гармошка белая два метра»)"],
    ["Товарная галерея", "витринный формат Директа с картинками товаров"],
    ["РСЯ", "баннеры на сайтах-партнёрах Яндекса"],
    ["Ретаргетинг", "показ рекламы тем, кто уже был на сайте"],
    ["Минус-слова", "слова, при которых реклама не показывается"],
    ["Аукцион", "торги за рекламный показ: кто больше платит и чьё объявление интереснее, того показывают выше"],
    ["Атрибуция", "как мы определяем, какой канал привёл заявку. У нас сверка не закончена с 8 июля, поэтому по регламенту ставки и бюджеты трогать нельзя. В августе это правило один раз нарушили, см. раздел 03"],
    ["ИИ-ответы", "сколько раз сайт всплыл в ответе нейросети внутри поиска"],
  ];
  return (
    <section className="sec sec--tight">
      <span id="slovar" aria-hidden="true" />
      <Reveal>
        <details className="gloss" open={false}>
          <summary>
            <span className="gloss__t">Обозначения простыми словами</span>
            <span className="gloss__h">открыть, если встретится незнакомое слово</span>
          </summary>
          <dl className="gloss__list">
            {terms.map(([t, d]) => (<div key={t}><dt>{t}</dt><dd>{d}</dd></div>))}
          </dl>
          <p className="gloss__foot">
            <b>Приоритеты задач:</b> <span className="p-P0">P0</span> блокирующее, делаем первым ·
            <span className="p-P1"> P1</span> первые две недели сентября ·
            <span className="p-P2"> P2</span> до конца сентября.<br />
            <b>Метки у цифр:</b> <T k="d" /> значение с указанным источником, <T k="h" /> оценка или цель, которую надо проверить.<br />
            <b>Кто исполнители:</b> Семён (поиск), Тимур (реклама), Макс (тексты), Виктор (скрипты продаж), Борис (данные и интеграции) - роли ИИ-системы GENGROUP. Дмитрий - разработчик сайта, человек. Иван - решения по деньгам и запуску.
          </p>
        </details>
      </Reveal>
    </section>
...
export function Glavnoe() {
  const items = [
    ...
    ["Бюджет за месяц не сократился, он перетёк.",
      "Товарную галерею остановили 16 августа, и это правильно, но экономии не вышло: за 14 дней после потрачено почти столько же, сколько за 16 дней до. Деньги перетекли на Поиск, а в тот же день включили новую кампанию РСЯ."],
  ];
  return (
    ...
    <section className="sec">
      ...
      <Reveal>
        <div className="note note--good">
          ...
          <p>{D.EFFORT_NOTE} <T k="h" /></p>
          <p><b>Чем мерить отдачу от этих дней.</b> Сегодня связать человеко-дни с заявками нечем: стоп-правила по страницам меряют
            позиции в выдаче, а весь остальной документ меряет заявки. Недостающее звено даёт задача 3: сколько заявок сегодня приносят
            небрендовые запросы. Эта цифра и станет точкой отсчёта, с которой в ноябре сравним результат.</p>
        </div>
      </Reveal>
      ...
      <Reveal>
        ...
        <div className="stack">
          ...
          <div className="stack__i"><i>d</i><p><b>И два разных сборщика расходятся сильнее.</b> По визитам из бесплатного поиска один даёт 2 704, другой 2 127, разница 27%, при этом на общей сумме визитов по сайту они сходятся точно: 8 611 в обоих. Таблица выше считает по большему числу, то есть по тому, которое даёт бесплатному поиску худшую долю обращений. При неразрешённом расхождении мы намеренно считаем по знаменателю, работающему против нашей же рекомендации <span className="tagD">ДАННЫЕ</span>.</p></div>
        </div>
      </Reveal>
      <div className="rule" />
    </section>
  ...
  return (
    <section className="sec">
      ...
      <Reveal>
            ...
            <tbody>
              {D.CAMPAIGNS.map((c) => (
                <tr key={c.name}>
                  ...
                  <td className={"n " + (c.name.startsWith("Поиск") || c.name.startsWith("Товарная") ? (c.ctr < 3 ? "bad" : "") : "dim")}>{pct(c.ctr, 2)}</td>
        ...
        <p className="chart__n">Итоговая кликабельность 1,47% средней по строкам не является: это частное сумм, а не среднее процентов. Она ниже каждой из четырёх кампаний на Поиске и в Товарной галерее и выше только РСЯ (0,32%) и ретаргетинга (0,74%). Причина в знаменателе: {fmt(68647)} из {fmt(D.IMPRESSIONS_TOTAL)} показов приходятся на баннеры РСЯ. Сравнивать надо строки между собой, а не строку с итогом. Кликабельность РСЯ и ретаргетинга показана серым: у баннеров и у показов вдогонку она по природе формата на порядок ниже, чем на Поиске, и с поисковыми строками не сравнивается. Ретаргетинг, судя по названию кампании, оплачивается за конверсии, а не за клики, поэтому у него ноль в расходе при 36 переходах.</p>
      </Reveal>
...
export function Poisk() {
  return (
    <section className="sec">
      ...
      <Reveal><p>{D.HUB_VS_DOSKI} <T k="d" /></p></Reveal>
      <Reveal>
        <div className="tw">
          <table>
...
export function Rynok() {
  return (
    <section className="sec">
      ...
      <Reveal>
        <div className="note note--alarm">
          <b className="note__t">Признание: в прошлой версии я написал здесь обратное</b>
          <p>Стояло «фильтры каталога у них почти не работают: девять адресов, семь с нулём, 4 единицы из 1 042, поэтому копировать
            соседа без проверки результата не надо». Проверка была неверной: я смотрел первую страницу выгрузки из девяти,
            отсортированную по алфавиту адреса, и все фильтры после точки обрыва в неё не попали. По полному срезу вывод обратный,
            и задача про фильтры поднята с последнего места в приоритетные.</p>
        </div>
      </Reveal>
      <Reveal>
        <div className="note">
          <b className="note__t">Что у loftcase устроено иначе</b>
          <p>Два коротких адреса под весь смысл вместо нашей россыпи из пятидесяти одной страницы. Отдельные страницы под конкретную ситуацию
            в квартире: «перегородка между кухней и коридором» даёт им 54 единицы одной страницей. И гардеробные, которых у нас нет вообще.</p>
          <p><b>Главное же у них - фильтры каталога.</b> Тридцать восемь фильтровых адресов дают {fmt(D.LOFTCASE_FILTERS.vis)} единиц,
            это половина видимости всего их сайта. Три из четырёх сильнейших перегородочных страниц у них именно фильтры:
            «раздвижные из стекла» 399, «по комнате гардеробная» 297, «раздвижные» 165. Двадцать два фильтра из тридцати восьми
            при этом имеют нулевую заметность - это цена подхода, и она окупается <T k="d" />.</p>
        </div>
      </Reveal>
      <div className="rule" />
    </section>
  ...
  return (
    <section className="sec">
      ...
      <Reveal>
        ...
        <p className="chart__n">fdmebel.ru в таблице нет: в нашем замере 31.08 у него ноль рекламных запросов и ноль объявлений. Про наши {M.ourAds}: это уникальные объявления, собранные по шести файлам выгрузки, каждый из которых первая страница из пяти-четырнадцати. В сводке keys.so у нас {M.ourAdsTotal} объявлений, то есть {M.ourAds} это нижняя граница, а не весь кабинет.</p>
      </Reveal>
      <Reveal>
        <div className="note note--alarm">
...
const S_LABEL: Record<string, [string, string]> = {
  ok: ["сделано", "good"], half: ["частично", "mid"], saved: ["сделано, но денег не сэкономило", "mid"],
  hold: ["держится", "good"], held: ["не запускалось", "mid"], no: ["не сделано", "bad"],
  down: ["страница просела", "bad"], unknown: ["проверить нечем", "mid"],
};
  ...
  return (
    <section className="sec">
      ...
      <Reveal>
        <div className="tw">
          <table>
            <thead><tr><th style={{ width: "3rem" }}>ID</th><th>Задача</th><th>Срок по плану</th><th>Статус</th><th>Чем проверено</th></tr></thead>
            <tbody>
              {D.PLAN_STATUS.map((p) => {
                const [l, cls] = S_LABEL[p.s];
                return (
                  <tr key={p.id}>
                    <td className="mono">{p.id}</td><td>{p.t}</td><td className="dim">{/^P\d$/.test(p.due) ? <span title="срок не назначался, был задан только приоритет">без даты, {p.due}</span> : p.due}</td>
                    <td className={cls}>{l}</td>
      ...
      <Reveal>
        <div className="note">
          ...
          <p>В плане {c.total} задач, здесь проверены {c.checkedTasks} задачи в {c.checkedRows} строках. Статус {c.unchecked} пунктов не разбирался,
            причём F1 и F2 сам план помечает как повтор уже проверенных, а E0 он же называет не задачей, а условием, то есть реально неизвестен статус {c.trulyUnknown}.
      ...
      <Reveal>
        <div className="note note--alarm">
          <b className="note__t">Счёт по {D.PLAN_ROWS} строкам таблицы, ярлыками самой таблицы</b>
          <p>{D.PLAN_TALLY.map((x) => `${x.label} ${x.n}`).join(", ")}. Одна строка - одна задача, поэтому строк и задач поровну: {D.PLAN_ROWS}.
            Укрупнять эти ярлыки в «сделано» и «не сделано» я не стал: у «сделано, но денег не сэкономило» и «не запускалось»
            нет однозначного места между ними, а от того, куда их отнести, менялся бы итог.</p>
          <p>Четыре из невыполненных были помечены как первоочередные со сроком в июле: техаудит доступа роботов, склейка дублей,
  ...
  return (
    <div className="task" data-open={open}>
      ...
      <div className="task__body"><div className="task__inner"><div className="task__wrap">
        ...
        <h4>Готово, когда</h4><p>{t.done} {t.doneTag && <T k={t.doneTag} title="Целевое число поставлено по уровню конкурентов или по нашей же соседней странице, замером не обосновано" />}</p>
  ...
  return (
    <section className="sec">
      ...
      <Reveal>
        ...
        <div className="stack">
          {D.NOT_DOING.map((n, i) => (
            <div className="stack__i" key={i}><i>×</i><p><b style={{ fontWeight: 600 }}>{n.t}.</b> {n.why} {n.t.includes("ссылки") && <T k="h" title="Сумма пилота взята из плана 21.07, ценами площадок не сверена" />}</p></div>
          ))}
        </div>
      </Reveal>
      <div className="rule" />
  ...
  return (
    <section className="sec">
      ...
      <Reveal>
        ...
        <p>{D.RAMP_NOTE} <span className="tag tag--h" title="Гипотеза, широкий диапазон: разброс втрое, срок у нас не измерялся; планировать надо по верхней границе">гипотеза, широкий диапазон</span></p>
        <div className="stack">
...
export function Foot() {
  return (
    <footer className="foot">
      <Reveal>
        <div className="sec__n">происхождение</div>
        <div className="verdict">
          <p style={{ maxWidth: "62ch" }}>Первоисточник этой страницы - текстовый отчёт <a href="../seo/peregorodki-audit.html" style={{ borderBottom: "1px solid var(--brass)" }}>peregorodki-audit.html</a>.
            Он прошёл адверсариальную приёмку ФЕНИКСА за три круга с оценками 6,25, 7,35 и 8,75 из 10; шестнадцать фактических ошибок найдены и исправлены до выпуска. Оценки существуют прозой в отчётах приёмки и в тексте коммитов, отдельного файла с ними в базе знаний пока нет.
            Эта страница проходит приёмку отдельно: её вердикт здесь не показан, потому что документ не должен сам себе выдавать оценку.</p>
        </div>
      </Reveal>
      <Reveal>
