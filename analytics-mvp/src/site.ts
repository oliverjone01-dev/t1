...
export function renderTovary(model: unknown): string {
  ...
  <div class="note">клик по модели раскрывает SKU и дневной тренд. <b>Маржа вал.</b> = (выручка-С\\С)/выручка, реагирует на период. <b>После сборов</b> = к начислению / выручка начисленная по транзакциям: после комиссии и услуг операции. НЕ включает канальную логистику/эквайринг (~3% оборота), поэтому на 6-7 п.п. выше полного М3 на странице Деньги (там 45.9% против ~52% здесь). Снимок 30 дней (не пересчитывается под период). Покрытие: 172 из ~340 SKU; комплектные (мульти-товарные) продажи не разнесены.</div>`;
  const js = `
