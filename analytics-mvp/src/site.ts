...
import { SVC_COLUMNS, SVC_OTHER } from "./scripts/ym/derive-lib.js";
...
function areaSvg(vals,stroke,labels,unit){const w=460,h=150,max=Math.max(1,...vals),n=vals.length,st=w/Math.max(1,n-1);const xy=vals.map((v,i)=>[i*st,h-(v/max)*(h-14)]);const pts=xy.map(p=>p[0].toFixed(1)+','+p[1].toFixed(1));const line='M'+pts.join(' L');const area=line+' L'+w+','+h+' L0,'+h+' Z';
  const dots=labels?xy.map((p,i)=>'<circle cx="'+p[0].toFixed(1)+'" cy="'+p[1].toFixed(1)+'" r="3.2" fill="'+stroke+'" fill-opacity="0.85"><title>'+labels[i]+' · '+rub(Math.round(vals[i]))+(unit||'')+'</title></circle>').join(''):'';
  return '<svg viewBox="0 0 '+w+' '+h+'" width="100%" height="150" preserveAspectRatio="none"><defs><linearGradient id="ag" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stop-color="'+stroke+'" stop-opacity=".30"/><stop offset="1" stop-color="'+stroke+'" stop-opacity="0"/></linearGradient></defs><path d="'+area+'" fill="url(#ag)"/><path d="'+line+'" fill="none" stroke="'+stroke+'" stroke-width="2.2"/>'+dots+'</svg>';}
let state={kind:'30',cur:null,cmp:null,compare:false};
// Проброс периода между страницами: выбор хранится в localStorage и восстанавливается на каждой странице.
const PKEY='gg_period_v1';
const gv=id=>{const el=document.getElementById(id);return el?el.value:'';};
function saveState(){try{localStorage.setItem(PKEY,JSON.stringify({kind:state.kind,compare:state.compare,cf:gv('cf'),ct:gv('ct'),pf:gv('pf'),pt:gv('pt')}));}catch(e){}}
function restoreState(){let s=null;try{s=JSON.parse(localStorage.getItem(PKEY)||'null');}catch(e){}if(!s||!s.kind)return;
  state.kind=s.kind;state.compare=!!s.compare;
  document.querySelectorAll('.chip').forEach(c=>c.classList.toggle('on',c.dataset.k===s.kind));
  const cr=document.getElementById('customRow');if(cr)cr.style.display=s.kind==='custom'?'flex':'none';
  if(s.cf&&document.getElementById('cf'))document.getElementById('cf').value=s.cf;
  if(s.ct&&document.getElementById('ct'))document.getElementById('ct').value=s.ct;
  const cb=document.getElementById('cmpBtn'),cm=document.getElementById('cmpRow');
  if(cb)cb.classList.toggle('on',state.compare);if(cm)cm.style.display=state.compare?'flex':'none';
  if(s.pf&&document.getElementById('pf'))document.getElementById('pf').value=s.pf;
  if(s.pt&&document.getElementById('pt'))document.getElementById('pt').value=s.pt;}
function swap(w){if(w&&w.from&&w.to&&w.from>w.to){const t=w.from;w.from=w.to;w.to=t;}return w;}
function apply(){let cur,cmp;if(state.kind==='custom')cur=swap({from:document.getElementById('cf').value,to:document.getElementById('ct').value});else cur=periodFor(state.kind);if(!cur.from||!cur.to){document.getElementById('period').textContent='укажите обе даты периода';return;}if(state.compare){cmp=swap({from:document.getElementById('pf').value,to:document.getElementById('pt').value});if(!cmp.from||!cmp.to)cmp=prevEqual(cur.from,cur.to);}else cmp=prevEqual(cur.from,cur.to);state.cur=cur;state.cmp=cmp;document.getElementById('period').textContent=cur.from+'..'+cur.to+'  против  '+cmp.from+'..'+cmp.to+(cmp.clip?' · неполная база сравнения (обрезана началом истории '+FLOOR+')':'');saveState();draw(cur,cmp);
  const note='период '+cur.from+'..'+cur.to+' · сравнение с '+cmp.from+'..'+cmp.to+(cmp.clip?' (обрезано началом истории)':'');
  document.querySelectorAll('.cmpnote').forEach(e=>{e.textContent=note;});}
...
function boot(staticFn){if(staticFn)staticFn();const dq=document.getElementById('dq');if(dq){const fr=DATA.fresh||{};dq.textContent='история по '+MAX+' (слой T-1)'+(fr.partial?' · последний день может быть неполным - сравнение занижает текущий период':'');}initControls();restoreState();apply();}
`;
...
export function renderTovary(model: unknown): string {
  const sections = `
  <h2>Срез периода</h2><div class="grid" id="kpis"></div><div class="note cmpnote"></div>
  <div class="two">
    ...
    <div><h2>BCG по линиям (рост × доля)</h2><div class="card"><svg id="bcg" viewBox="0 0 460 260" width="100%" height="260"></svg><div class="note cmpnote"></div></div></div>
  </div>
...
function draw(cur,cmp){
  ...
  const spark=(ser,labs)=>{const w=120,h=26,mx=Math.max(1,...ser),st=w/Math.max(1,ser.length-1);const p=ser.map((v,i)=>i*st+','+(h-(v/mx)*(h-3))).join(' ');const dots=(labs||[]).map((d,i)=>ser[i]>0?'<circle cx="'+(i*st).toFixed(1)+'" cy="'+(h-(ser[i]/mx)*(h-3)).toFixed(1)+'" r="2.4" fill="#FF4438"><title>'+d+' · заказов: '+ser[i]+'</title></circle>':'').join('');return '<svg width="'+w+'" height="'+h+'" viewBox="0 0 '+w+' '+h+'"><polyline fill="none" stroke="#FF4438" stroke-width="1.6" points="'+p+'"/>'+dots+'</svg>';};
  ...
  groups.forEach((g,li)=>{const mm=g.margin!=null?g.margin+'%':na,tm=g.after!=null?g.after+'%':na;
    ...
    g.sk.forEach(s=>{const c=cogsOf(s.sku);const sm=(c>0&&s.rev>0)?(Math.round((s.rev-c*s.units)/s.rev*1000)/10)+'%':na;const cTxt=c>0?'С\\\\С '+rub(c)+' ₽':'нет С\\\\С';const tt=txTake(s.sku);
      ...
      rh+='<tr class="sku-row" data-li="'+li+'" style="display:none"><td colspan="9" style="padding-top:0"><span class="sub">'+cTxt+' · тренд заказов по дням: </span>'+spark(series(s.sku,cur.from,cur.to),between(cur.from,cur.to))+'</td></tr>';});});
...
function draw(cur,cmp){
  ...
  const dr=dailyRev(cur.from,cur.to);document.getElementById('trend').innerHTML=areaSvg(dr.map(d=>d.rev),tc.rev>=tp.rev?'#34D399':'#FF4438',dr.map(d=>d.date),' ₽');
...
export function renderMoney(model: unknown): string {
  ...
  const svodHead = IS_OZON ? "" : `
  <h2>Свод по дате заказа &middot; доставлено минус отмены и возвраты</h2>
  <div class="card" style="padding:12px 14px"><div style="display:flex;gap:10px;flex-wrap:wrap;align-items:center">
    <label class="sub">Кабинет <select id="svB" style="margin-left:6px"></select></label>
    <label class="sub">Месяц заказа <select id="svM" style="margin-left:6px"></select></label>
    <span class="sub" id="svCov"></span>
  </div></div>
  <div class="wgrid" id="svKpi" style="margin-top:10px"></div>
  ...
  <b>Выручка деньгами</b> = платёж покупателя (уже за вычетом скидки Маркета и баллов Плюса) + доставка с покупателя - возвраты. Цена продажи показана справочно и в результат не идёт: она включает скидку, которую платил Маркет, а не покупатель.<br>
  <b>Услуги</b> разделены на оплаченные деньгами и оплаченные баллами Маркета. Обе половины - реальный расход: услугу оказали и в акте она стоит полностью. Результат «по деньгам» показывает, что осталось на счёте; результат «с учётом баллов» - экономику заказа целиком.</div>`;
  const sections = `
  ${closedHead}${svodHead}
...
var SV=(DATA.svod&&DATA.svod.months)||[];
var SVB={"74986385":"GEN GROUP (мебель)","1023124":"GENGLASS (зеркала)"};
var SVC_ORDER=${JSON.stringify(SVC_COLUMNS.map((c) => c[0]).concat([SVC_OTHER]))};
function svName(b){return SVB[b]||('кабинет '+b);}
function svPick(){var b=document.getElementById('svB').value,m=document.getElementById('svM').value;
  return SV.filter(function(x){return (b==='all'||x.business===b)&&x.ym===m;});}
function svSum(ms,f){var t=0;ms.forEach(function(m){m.rows.forEach(function(r){t+=f(r)||0;});});return Math.round(t);}
...
function svDraw(){
  ...
  var orders=0,noLed=0,acts={},cov=0,covW=0,unset=0;
  ms.forEach(function(m){orders+=m.orders;noLed+=m.orders_without_ledger;if(!m.svc_settled)unset++;(m.svc_months||[]).forEach(function(a){acts[a]=1;});
  ...
  var ohM=0,ohP=0;ms.forEach(function(m){ohM+=m.overhead_money||0;ohP+=m.overhead_points||0;});
  var cogs=svSum(ms,function(r){return r.cogs;});
  ...
  document.getElementById('svKpi').innerHTML=[
    ...
    svCard('Баллы начислены Маркетом',pts,'компенсация скидок Маркета','#8AA0FF'),
    svCard('Услуги деньгами',-(sm+Math.round(ohM)),'списано со счёта, с общими расходами','#FF5A5F'),
    svCard('Услуги баллами',-(sp+Math.round(ohP)),'оплачено баллами, денег не списали','#F2B544'),
    ...
    svCard('Валовая прибыль',gross,'результат с баллами - С\\С',gross>=0?'#34D399':'#FF5A5F')].join('');
  ...
  // услуги: деньги vs баллы по статьям
  var m0={},p0={};SVC_ORDER.forEach(function(c){m0[c]=0;p0[c]=0;});
  ms.forEach(function(m){m.rows.forEach(function(r){SVC_ORDER.forEach(function(c){m0[c]+=(r.svc||{})[c]||0;p0[c]+=(r.svc_pts||{})[c]||0;});});
    for(var k in (m.overhead||{}))m0[k]=(m0[k]||0)+m.overhead[k];for(var k2 in (m.overhead_pts||{}))p0[k2]=(p0[k2]||0)+m.overhead_pts[k2];});
  var sh='<table><thead><tr><th>Статья услуги</th><th class="r">Деньгами</th><th class="r">Баллами</th><th class="r">Всего</th><th class="r">% выручки</th></tr></thead><tbody>';
  ...
  var agg={};ms.forEach(function(m){m.rows.forEach(function(r){var k=r.sku;var a=agg[k];
    if(!a){a=agg[k]={sku:r.sku,name:r.name,ud:0,ur:0,un:0,price:0,ship:0,dmp:0,dpl:0,pay:0,ref:0,rev:0,pts:0,sm:0,sp:0,cogs:0,ck:r.cogs_known};agg[k]=a;}
  ...
  list.forEach(function(a){var tot=a.sm+a.sp,rm=a.rev-a.sm,rp=a.rev+a.pts-tot,gp=a.ck?rp-a.cogs:null;
    th+='<tr><td class="num">'+a.sku+(a.ck?'':' <span class="pill b-Z" title="нет себестоимости">нет С\\С</span>')+'</td><td>'+(a.name||'').slice(0,46)+'</td>'+
      '<td class="r num">'+a.ud+'</td><td class="r num">'+a.ur+'</td><td class="r num">'+a.un+'</td>'+
      '<td class="r num sub">'+rub(a.price)+'</td><td class="r num">'+rub(a.ship)+'</td><td class="r num sub">'+rub(a.dmp)+'</td><td class="r num sub">'+rub(a.dpl)+'</td>'+
      '<td class="r num">'+rub(a.pay)+'</td><td class="r num down">'+rub(a.ref)+'</td><td class="r num"><b>'+rub(a.rev)+'</b></td>'+
      '<td class="r num" style="color:#8AA0FF">'+rub(a.pts)+'</td>'+
...
function answer(q){
  ...
  let h='<div style="font-size:15px;line-height:1.7">Оборот <b>'+mln(tc.rev)+' ₽</b> ('+pct(dg(tc.rev,tp.rev))+' к пред.), заказов <b>'+tc.units+'</b> ('+pct(dg(tc.units,tp.units))+').<br>Валовая маржа <b>'+g.gm+'%</b> по С\\\\С (покрытие '+g.cov+'%). ДРР канала <b>'+${IS_OZON ? `(At.drr||0)+'%</b> (снимок 30д, отдельный период)` : `(At.spend>0&&!At.drr?'не считается (расход есть, привязки к заказам нет)':(At.drr||0)+'%')+'</b> (снимок 30д, отдельный период)`}.<br>Лучше всех растёт <b>'+(up?up.line:'-')+'</b> ('+(up?pct(up.growth):'')+'), хуже всех <b>'+(dn?dn.line:'-')+'</b> ('+(dn?pct(dn.growth):'')+').<br>Возвратов '+tc.ret+', SKU с продажами '+tc.sku+'.</div>';
