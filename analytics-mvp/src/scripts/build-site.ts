// Собирает страницу "Товары" со встроенной историей и клиентским движком.
...
function main() {
  ...
  // справочник SKU: sku -> [name, line]
  const skus: Record<string, [string, string]> = {};
  for (const r of rows) if (!skus[r.sku]) skus[r.sku] = [r.name, r.line];

  // компактные факты: [date, sku, revenue, units, views, returns]
  const facts = rows.map((r) => [r.date, r.sku, Math.round(r.revenue), r.units, r.views, r.returns]);
  ...
  console.log(`Готово: public/tovary.html · фактов ${facts.length} · SKU ${Object.keys(skus).length} · ${model.floor}..${model.max}`);
}
