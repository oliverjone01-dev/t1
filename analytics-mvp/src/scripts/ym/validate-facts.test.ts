...
// Источник, которого нет вовсе. Отчёта по баллам Маркета в Partner API не существует, поэтому
// bonuses_monthly.ndjson не создаётся никогда - а гейт ронял на нём прогон («пуст или нечитаем»)
// и блокировал публикацию всего остального, включая только что заработавший акт услуг.
// Живой случай 2026-09-16, прогон 35083444139: акт разобрался на 6 588 строк и не доехал до
// коммита из-за файла, которого не может быть.
describe("источник не собран: отличать «не было никогда» от «исчезло»", () => {
  const missing = join(DIR, "never-collected.ndjson");   // файл не создаём намеренно

  it("файла нет и раньше не было - сторожить нечего, прогон идёт дальше", () => {
    expect(gate(missing, "", KEY)).toBe(0);
  });

  it("файла нет, а в прошлом снимке он был - это потеря, блок", () => {
    expect(gate(missing, BASE, KEY)).toBe(1);
  });

  it("файл есть, но пустой - по-прежнему блок (это порча, а не отсутствие источника)", () => {
