...
// --- страница 4: Маркетинг (период МГНОВЕННО из запечённых снимков 7/30/90 + живое обновление) ---
{
  ...
  <section class="card"><div class="card-h"><div><div class="card-title">Юнит-экономика рекламы: ДРР vs безубыток</div><div class="card-sub">Лимит РК (безубыточная ДРР) = 100% − все сборы OZON (комиссия+логистика+эквайринг+хранение) − себестоимость продвигаемого SKU. Запас = Лимит РК − фактическая ДРР кампании. Решение: 🟢 запас ≥30% лимита · 🟡 0…30% · 🔴 &lt;0. Расход/выручка/ДРР - по всей кампании. Ср. цена, Комис. (₽/%), С/с (₽/%) SKU - из снимка 30 дней и НЕ зависят от выбранного периода (per-SKU финансов по дням пока нет); период фильтра влияет на Расход / Выручка / ДРР. Per-единица: цена = выручка ÷ штуки, комиссия ₽/шт = цена × комис.%, с/с ₽/шт - себестоимость единицы (sku_cogs). Светофор приоритизации «газ/режь», не P&amp;L до копейки.</div></div></div><div class="kt-kpi" id="uecon-kpi" style="margin-bottom:10px"></div><div class="kt-scroll"><table class="kt-table"><thead><tr><th>Кампания / SKU</th><th class="r" title="Расход кампании на рекламу за выбранный период. Источник: OZON Performance (дневной ряд statistics/daily или снимок).">Расход</th><th class="r" title="Выручка, атрибутированная рекламе кампании за период (заказы из продвижения). Источник: OZON Performance.">Выручка рекл.</th><th class="r" title="ДРР = Расход ÷ Выручка рекл. × 100. Источник: OZON Performance.">ДРР</th><th class="r" title="Ср. цена единицы = выручка SKU ÷ проданные штуки. Источник: снимок 30 дней (выручка - pnl_sku, штуки - sku_table).">Ср. цена</th><th class="r" title="Комиссия OZON на единицу, ₽ = Ср. цена × Комис.%. Источник: снимок 30 дней.">Комис., ₽</th><th class="r" style="color:var(--ink-3)" title="Комис.% = все сборы OZON = (начислено − к выплате) ÷ начислено × 100. Источник: pnl_sku снимок 30 дней (начислено accruals, к выплате amount).">Комис., %</th><th class="r" title="Себестоимость единицы, ₽. Источник: ручная карта sku_cogs.json.">С/с, ₽</th><th class="r" style="color:var(--ink-3)" title="С/с% = доля себестоимости = С/с,₽ × штуки ÷ выручку × 100. Источник: sku_cogs × штуки (снимок 30д) ÷ выручка.">С/с, %</th><th class="r" title="Лимит РК (безубыточная ДРР) = 100 − Комис.% − С/с%. Сколько можно тратить на рекламу, не уходя в минус.">Лимит РК</th><th class="r" title="Запас = Лимит РК − фактическая ДРР, в процентных пунктах. >0 - реклама в плюс.">Запас, п.п.</th><th>Решение</th></tr></thead><tbody id="uecon"></tbody></table></div></section>
  ...
  const pageJs = `
const SNAP=${J(adsSnap)};const PRICE=${J(PRICE)};const PERIODS=${J(adsPeriods)};const RCACHE=${J(adsReports)};const SKU_MAP=${J(skuMap)};const SKU_CAT=${J(skuCat)};const ECON=${J(SKU_ECON)};const SKUS_BY_CAMP=${J(skuByCamp)};const CARDG=${J(cardBySku)};const ADS_DAILY=${J(adsDaily)};const ATTR_DAILY=${J(adsAttrByCamp)};const CAMP_META=${J(campMeta)};const CAMP_CARD=${J(campCard)};
...
function renderUecon(a){
  ...
  (a.top_spend||[]).forEach(function(c){
    ...
    rows.push({camp:c.id,art:art,status:c.status,sp:sp,om:om,drr:drr,com:com,cogs:e.cogs,price:e.price,comRub:e.comRub,cogsRub:e.cogsRub,be:be,head:head,v:v});
  });
  var el=document.getElementById('uecon');var elk=document.getElementById('uecon-kpi');if(!el)return;
  if(!total){el.innerHTML='<tr><td colspan="12" class="kt-note">нет кампаний с продвигаемым SKU за период</td></tr>';if(elk)elk.innerHTML='';return;}
  ...
  el.innerHTML=rows.map(function(r){
    ...
    if(r.na)return '<tr style="opacity:.65">'+nm+'<td class="r">'+fmtRu(r.sp)+'</td><td class="r">'+fmtRu(r.om)+'</td><td class="r">'+(r.drr||0)+'%</td><td class="r" colspan="7" style="color:var(--ink-3)">⚪ н/д: '+r.why+'</td><td>не считаем</td></tr>';
