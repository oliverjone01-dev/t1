...
if [ "$HAS_AUDIT" -eq 1 ]; then
  DRIFT="$(bash .claude-plugin/sync-agents.sh --check 2>/dev/null | grep -E '^(ДРЕЙФ|лишний)' || true)"
  if [ -n "$DRIFT" ]; then
    python3 - "$DRIFT" <<'PY'
import json, sys
print(json.dumps({"hookSpecificOutput": {"hookEventName": "PreToolUse", "additionalContext": "Плагин gengroup-roster: копии агентов разошлись с источником. " + sys.argv[1] + " Запусти bash .claude-plugin/sync-agents.sh до push."}}, ensure_ascii=False))
PY
  fi
  exit 0
fi
