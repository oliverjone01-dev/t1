...
счетам** и **поступления за операционный день** и шлёт сводку в Telegram.
...
**Конфиденциальность:** снимок (остатки, номера счетов) НЕ коммитится в git - он
живёт только внутри прогона и уходит в приватный Telegram. `data/` в `.gitignore`,
воркфлоу работает с правами `contents: read` (пушить не может физически).
...
```
tochka-bank/
  fetch.mjs        - клиент API: auth -> счета -> баланс -> выписка -> data/latest.json (эфемерно)
  notify.mjs       - шлёт сводку (остаток + поступления) в Telegram
  accounts.json    - псевдонимы счетов (последние 4 цифры -> название); только отображение
  data/            - снимок прогона, в git НЕ попадает (.gitignore)
```

1. `.github/workflows/tochka-snapshots.yml` (ежедневный cron ~09:30 МСК) запускает
   `node fetch.mjs` -> дельта-гейт -> `node notify.mjs`. Снимок нигде не сохраняется.
