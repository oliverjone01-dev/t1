...
# Голые «сегодня», «завтра» тоже конкретика: «завтра отправлю» проверяемо, и
# считать его «обещанием без даты» значит собрать треть ложных срабатываний на
# главном счётчике (замер ФЕНИКСА 10.08.2026: 216 из 671 содержали ориентир).
CONCRETE = re.compile(
    ...
    r"|\b\d{1,2}\.\d{1,2}(?:\.\d{2,4})?\b|\b(?:сегодня|завтра|послезавтра)\b"
    r"|до\s+конца\s+(?:дня|недели)"
