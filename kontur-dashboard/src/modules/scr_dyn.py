...
'dyn-ym': () => {
  ...
  return head('Визиты по дням', 'Данные Яндекс.Метрики, счётчик '+esc(String(ym.counter.v))+'. Ряд из '+nf(days.length)+' дней.',
   ...
   + ((ym.goals||[]).length ? '' : (ym.goals_configured||[]).length
      ? '<div class="mt-4">'+note('Цели настроены, эта выгрузка их не отдаёт',
          'На счётчике активны '+nf((ym.goals_configured||[]).length)+' целей (экран «Цели и конверсии»), но достижений по ним здесь нет: выгрузка geo-monster их не запрашивает. Мост визит-лид пока держится на бенчмарке не из-за отсутствия целей.','warn')+'</div>'
      : '<div class="mt-4">'+note('Целей в счётчике нет',
