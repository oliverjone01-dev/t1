...
p();
// Ноль срабатываний детектора это либо реальная чистота, либо сломанный детектор.
// Печатать его как достижение нельзя: отдел получит похвалу за контроль, которого нет.
if (!c.D06 && A.totals.lostInWindow) {
  p(`Детектор D06 (отказ без причины) дал 0 срабатываний на ${A.totals.lostInWindow} отказов за окно. ` +
    `Это либо реальная дисциплина, либо детектор перестал матчиться: проверить частотную таблицу ` +
    `значений поля «причина отказа» прежде чем считать это хорошей новостью.`);
  p();
}
