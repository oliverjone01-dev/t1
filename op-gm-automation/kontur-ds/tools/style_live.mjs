// Живая проверка стиля в браузере (1.5.1): те же признаки шаблонного интерфейса, что у сторожа, но по вычисленным
...
const scan = () => {
  ...
  for (const e of document.querySelectorAll('body *')) {
    ...
    const sideStripe = (st, who) => {
      ...
      for (const side of ['Left', 'Right']) {
        ...
        if (quiet('Top') && quiet('Bottom') && quiet(other)) out.push(['SIDESTRIPE', who + ' рамка ' + side.toLowerCase() + ' ' + w + 'px']);
      }
    };
    ...
    for (const part of cs.boxShadow === 'none' ? [] : cs.boxShadow.split(/,(?![^(]*\))/)) {
      ...
      if (nums.length >= 2 && Math.abs(nums[0]) >= 2 && nums[1] === 0 && !(nums[2] > 0) && !(nums[3] > 0) && colored(c)) out.push(['SIDESTRIPE', name(e) + ' тень ' + part.trim().slice(0, 40)]);
    }
    for (const ps of ['::before', '::after']) {
      ...
      if (w >= 1 && w <= 4 && h >= 16 && (colored(p.backgroundColor) || (gcol && colored(gcol)))) out.push(['SIDESTRIPE', name(e) + ps + ' полоса ' + w + 'x' + Math.round(h)]);
      sideStripe(p, name(e) + ps);
      if (p.textTransform === 'uppercase' && p.content && p.content !== 'none' && p.content !== '""' && !e.closest('.ks-caps')) out.push(['CAPSBADGE', name(e) + ps + ' капс в content']);
    }
    ...
    if (hg && +hg[2] <= 6 && colored(hg[1]) && hg[1] !== hg[3]) out.push(['SIDESTRIPE', name(e) + ' край градиента ' + hg[2] + 'px']);
    ...
    if (r.width >= 1 && r.width <= 4 && r.height >= 16 && colored(cs.backgroundColor) && !/resize/.test(cs.cursor)) {
      ...
      if (edgeChild && textNear && narrowSibs < 2 && !(par && par.querySelector('svg, canvas')))
        out.push(['SIDESTRIPE', name(e) + ' узкий блок ' + Math.round(r.width) + 'x' + Math.round(r.height)]);
    }
    ...
    if (hasText(e) && (cs.textTransform === 'uppercase' || /small-caps|petite-caps/.test(cs.fontVariantCaps)) && !recipe && !e.closest('.ks-caps, input, textarea, .ks-input'))
      out.push(['CAPSBADGE', name(e) + ' «' + e.textContent.trim().slice(0, 20) + '»']);
    ...
    if (markish(e) && /gradient\(/.test(cs.backgroundImage)) out.push(['GRADMARK', name(e)]);
    ...
    if (hasText(e) && !e.closest('[disabled], [aria-disabled="true"], .is-disabled, .ks-skeleton, .ks-tip, .ks-backdrop')) {
      ...
      if (a < 0.95 && a > 0.05) out.push(['OPACITY', name(e) + ' прозрачность ' + a.toFixed(2)]);
      else if (ca < 0.95 && ca > 0.05) out.push(['OPACITY', name(e) + ' цвет с прозрачностью ' + ca.toFixed(2)]);
    }
  }
  ...
  for (const sv of document.querySelectorAll('svg')) {
    ...
    if (grad) out.push(['GRADMARK', 'svg-знак с градиентом в ' + name(host)]);
  }
  ...
  for (const an of document.getAnimations()) {
    ...
    out.push(['LOOPANIM', (tg && tg.nodeType === 1 ? name(tg) : 'анимация') + ' ' + (an.animationName || 'element.animate')]);
  }
  for (const s of document.querySelectorAll('animate, animateTransform, animateMotion, set'))
    if (/indefinite/.test((s.getAttribute('repeatCount') || '') + (s.getAttribute('repeatDur') || '')) || +(s.getAttribute('repeatCount') || 0) >= 50) out.push(['LOOPANIM', 'SMIL ' + s.tagName]);
  return out;
};
...
const blinkScan = () => new Promise(done => {
  ...
  setTimeout(() => { mo.disconnect();
    const name = e => ((e.id ? '#' + e.id : '') + (typeof e.className === 'string' && e.className ? '.' + e.className.trim().split(/\s+/).slice(0, 2).join('.') : '') || e.tagName.toLowerCase()).slice(0, 48);
    done([...n].filter(([, k]) => k >= 3).map(([e, k]) => ['LOOPANIM', name(e) + ' переключается скриптом ' + k + ' раз за 2 с'])); }, 2000);
});
...
const tapScan = () => {
  ...
  for (const e of document.querySelectorAll('button, select, input:not([type="hidden"]), textarea, a[href], summary, [role="button"], [role="tab"], [role="switch"], [role="checkbox"], [role="radio"], [role="menuitem"], [role="option"], [tabindex]:not([tabindex="-1"]), [onclick], .ks-nav-item')) {
    ...
    if (side < 35.5) out.push(['DENSITYTAP', name(e) + ' ' + Math.round(r.width) + 'x' + Math.round(r.height)]);
  }
  return out;
};
...
for (const f of files) {
  const seen = new Set();
  const report = (code, where, what) => { const k = code + what; if (seen.has(k)) return; seen.add(k); total++; console.log(`   [${code}] ${path.basename(f)} ${where}: ${what}`); };
  for (const theme of ['light', 'dark']) {
    for (const width of [1280, 390]) {
      ...
      for (const [code, what] of await page.evaluate(scan)) report(code, `тема=${theme} ширина=${width}`, what);
      if (theme === 'light' && width === 1280) for (const [code, what] of await page.evaluate(blinkScan)) report(code, 'за 2 с после открытия', what);
      await page.close();
    }
  }
  ...
  for (const [code, what] of await page.evaluate(tapScan)) report(code, 'касание компакт 390', what);
  await ctx.close();
