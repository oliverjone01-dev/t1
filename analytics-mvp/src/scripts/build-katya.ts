...
function soDraw(){
  ...
  var H=['Категория / Заказ','Продажи','Оплатил клиент','Доставка покупателя'].concat(FEE)
    .concat(['Баллы Маркета','Штуки','Поступление','Наша доставка','С\\С произв.',
             ...
             'Кто везёт','Город']);
  var SO_PAID_TIP='Справочно, в расчёт не входит. Сколько заказ стоил САМОМУ ПОКУПАТЕЛЮ: его платёж за товар и доставку за вычетом возвратов. С «Продажами» не совпадает и совпадать не должен: там прайс целиком, вместе с долей Маркета - скидку на кассе Маркет платит за покупателя и возвращает нам баллами. Доставка покупателя уже внутри этого числа, складывать колонки нельзя.';
  var h='<thead><tr>'+H.map(function(x,i){
    return '<th'+(i?' class="r"':'')+(x==='Оплатил клиент'?' style="color:var(--ink-2)" title="'+SO_PAID_TIP+'"':'')+'>'+x+'</th>';}).join('')+'</tr></thead><tbody>';
