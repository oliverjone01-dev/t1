...
describe("свод Маркета: числа на странице", () => {
  ...
  it("июль воспроизводит выверенные числа до рубля", () => {
    ...
    // пишут один артикул с разными дефисами). СС июля +148 203 ₽. Валовая при этом ВЫРОСЛА, а
    // не упала: итог считается только по SKU с известной С\С, и строки без неё раньше выпадали
    // из расчёта целиком - вместе со своим поступлением. Теперь они внутри, отсюда и рост.
    // Чистая, наоборот, просела на 2 025 ₽: прибавка поступления меньше прибавки С\С с налогами.
    expect(num(cell("СС произв."))).toBe(2071046);
    expect(num(cell("Валовая прибыль"))).toBe(1742233);
    expect(num(cell("Чистая прибыль"))).toBe(26257);
  });
