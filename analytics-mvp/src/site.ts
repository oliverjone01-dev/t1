...
function apply(){let cur,cmp;if(state.kind==='custom')cur=swap({from:document.getElementById('cf').value,to:document.getElementById('ct').value});else cur=periodFor(state.kind);if(state.compare)cmp=swap({from:document.getElementById('pf').value,to:document.getElementById('pt').value});else cmp=prevEqual(cur.from,cur.to);state.cur=cur;state.cmp=cmp;document.getElementById('period').textContent=cur.from+'..'+cur.to+'  против  '+cmp.from+'..'+cmp.to;draw(cur,cmp);}
