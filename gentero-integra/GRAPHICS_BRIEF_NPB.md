...
Два кадра под разделы «Партнёр получает не скидку, а поток» и «Там, где
решение уже принимают». Слоты в вёрстке уже стоят и до появления файлов
...
|---|---|---|
| `gm-suz` | 1600 x 900 | Раздел 06, «Партнёр получает не скидку, а поток» |
| `gm-ritual` | 1600 x 900 | Раздел 07, «Там, где решение уже принимают» |
...
## Если понадобится третий кадр

Раздел про выставки сейчас живёт без своей фотографии и опирается на кадр
`gm-ritual`. Отдельный кадр стенда имеет смысл добавлять только если станет
понятно, что раздел на большом экране проседает. Промт под него:
...
Имя файла в этом случае `gm-expo`, размер 1600 x 900, строку в `SPEC` внутри
`prepare-images.py` придётся добавить руками.
