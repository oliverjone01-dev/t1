...
const WHO = {
  bridge: ['Иван', 'до 30.09.2026, конец отчётного месяца'],
...
const botLine = () => DB.bot
  ? ' Съём пишет бот kontur-snapshot.yml по будням.'
  : ' Бот съёма kontur-snapshot.yml записывает точки по будням только из main, и эта сборка сделана не им: пока он не работает, каждый будний день теряется точка по топ-50 и ответам ИИ.';
