...
export function coverageStrip(): string {
  ...
  // Иван 18.09.2026 прислал скрин этой полосы: «вот такие приписки скрой». Служебная сверка
  // нужна (§15 требует, чтобы пробел был виден), но не первым экраном поверх дашборда.
  // Поэтому вся полоса уезжает под одну неяркую строку-раскрывашку: мешать перестаёт,
  // доступна в один клик, ничего не потеряно.
  return `<div id="gg-cov" style="background:#141c26;border-bottom:1px solid #2a3a4a;color:#cfe8ef;font:12px/1.6 system-ui;padding:4px 18px">
  <details><summary style="cursor:pointer;color:#7d93a6;font-size:11.5px;list-style:none">служебное: сверка §15 ${isGo ? "GO" : "RETURN"} и покрытие данных</summary>
  <div style="padding-top:6px">${warn}
  ...
  <details style="display:inline-block;margin-left:6px"><summary style="cursor:pointer;color:#22D3EE">три периода и кумулятив</summary>${per}${cum}${apBlock}${fees}</details></div></div></details></div>`;
}
