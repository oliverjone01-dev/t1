...
function renderUecon(a){
  ...
  (a.top_spend||[]).forEach(function(c){
    ...
    rows.push({camp:c.id,art:art,status:c.status,sp:sp,om:om,drr:drr,ord:ord,com:com,cogs:cogsPct,price:price,priceClient:priceClient,comRub:comRub,cogsRub:cogsRub,profit:profit,be:be,head:head,v:v});
  });
  var el=document.getElementById('uecon');var elk=document.getElementById('uecon-kpi');if(!el)return;
  if(!total){el.innerHTML='<tr><td colspan="14" class="kt-note">нет кампаний с продвигаемым SKU за период</td></tr>';if(elk)elk.innerHTML='';return;}
  ...
  el.innerHTML=rows.map(function(r){
    ...
    if(r.na)return '<tr style="opacity:.65">'+nm+'<td class="r">'+fmtRu(r.sp)+'</td><td class="r">'+fmtRu(r.om)+'</td><td class="r">'+(r.drr||0)+'%</td><td class="r" colspan="9" style="color:var(--ink-3)">⚪ н/д: '+r.why+'</td><td>не считаем</td></tr>';
